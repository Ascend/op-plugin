...
TorchNPU使用monkey-patch方式与PyTorch接口进行对接，即将TorchNPU部分接口动态替换至PyTorch接口中，使用户在昇腾NPU上可以继续使用熟悉的PyTorch接口。

TorchNPU项目采用C++和Python联合开发，当前正式对外接口仅包括Python接口。C++接口为内部使用接口，不建议用户使用。
...
> [!NOTE]  
> TorchNPU部分功能可以通过环境变量实现，具体可参考《[环境变量参考](https://gitcode.com/Ascend/pytorch/blob/v2.7.1-26.1.0/docs/zh/environment_variable_reference/env_variable_list.md)》。
