...
基于PyTorch2.12.0/2.11.0/2.10.0/2.9.0/2.7.1版本，提供Ascend Extension for PyTorch自定义API的功能说明、函数原型、参数说明与调用示例等。
...
所有自定义API默认支持Ascend Extension for PyTorch版本匹配的全量PyTorch版本，若非全量支持会在各API的约束说明进行标注。

> [!NOTE]  
> Ascend Extension for PyTorch部分功能可以通过环境变量实现，具体可参考《[环境变量参考](https://gitcode.com/Ascend/pytorch/blob/v2.7.1-26.1.0/docs/zh/environment_variable_reference/env_variable_list.md)》。
