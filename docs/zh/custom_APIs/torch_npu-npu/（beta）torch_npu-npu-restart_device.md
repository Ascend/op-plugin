...
- **rebuild_all_resources** (`bool`)：可选参数，是否重建该device上的资源，默认为False。设置为True时，会恢复该device上的所有NPU stream，并联动`disable_tensor_unsafe_check`决定是否进行数据置脏。
- **disable_tensor_unsafe_check** (`bool`)：可选参数，是否禁用数据标脏，默认为False。仅在`rebuild_all_resources=True`时生效。为False时将该device上的所有NPU tensor进行标记为脏；为True时跳过标脏。
