...
- **API功能**：完成量化的Matmul计算、Permute（保证通信后地址连续）和AlltoAll通信的融合，先计算后通信。支持K-C量化模式（即pertoken-perchannel量化模式）和mx量化模式（即特殊的pergroup-pergroup量化模式）。

- **计算公式**：假设输入`x1`的shape为\(BS, H1\)，输入`x2`的shape为\(H1, H2\)，rankSize为NPU卡数。
  - K-C量化模式：
  ...
  - mx量化模式：
    ...
    $$
    computeOut = \sum_{0}^{\frac{k}{blockSize}=32}(x1@x2*(x1Scale*x2Scale)) + bias
    $$
...
```python
torch_npu.npu_quant_matmul_all_to_all(x1, x2, hcom, world_size, bias=None, x1_scale=None, x2_scale=None, common_scale=None, x1_offset=None, x2_offset=None, x1_quant_mode=None, x2_quant_mode=None, common_quant_mode=None, group_sizes=None, all2all_axes=None, comm_quant_dtype=None, x1_dtype=None, x2_dtype=None, x1_scale_dtype=None, x2_scale_dtype=None, output_scale_dtype=None, comm_scale_dtype=None, y_dtype=None, comm_mode=None) -> Tensor
```

## 参数说明

- **x1**（`Tensor`）：**必选参数**，表示融合算子的左矩阵输入，也是Matmul计算的左矩阵，对应公式中的x1。数据类型支持`torch.float8_e5m2`、`torch.float8_e4m3fn`、`torch_npu.float4_e2m1fn_x2`，维度只能为2D，shape为\(BS, H1\)，数据格式支持$ND$，**不支持非连续Tensor**。
- **x2**（`Tensor`）：**必选参数**，表示融合算子的右矩阵输入，也是Matmul计算的右矩阵，对应公式中的x2。数据类型支持`torch.float8_e5m2`、`torch.float8_e4m3fn`、`torch_npu.float4_e2m1fn_x2`，维度只能为2D，shape为\(H1, H2\)，数据格式支持$ND$，支持转置非连续Tensor。
- **hcom**（`str`）：**必选参数**，Host侧标识列组的字符串，即通信域名称，通过get\_hccl\_comm\_name接口获取。
- **world\_size**（`int`）：**必选参数**，通信域内的rank总数，对应公式中的rankSize，支持范围\[2，4，8，16\]，CCU仅支持单机UB域内互联，AI CPU可支持跨机UB域内互联。
- **bias**（`Tensor`）：**可选参数**，矩阵乘运算后累加的偏置，对应公式中的bias。数据类型支持`torch.float32`，维度只能为1D，shape为\(H2\)，数据格式支持$ND$。
- **x1\_scale**（`Tensor`）：**可选参数**，表示左矩阵的量化系数，对应公式中的x1Scale。K-C量化场景下，数据类型支持`torch.float32`，维度为1D，shape为\(BS\)，数据格式支持$ND$。mx量化场景下，数据类型支持`torch_npu.float8_e8m0`，维度为3D，shape为\(BS, ceil\(H1/64\), 2\)，数据格式支持$ND$。
- **x2\_scale**（`Tensor`）：**可选参数**，表示右矩阵的量化系数，对应公式中的x2Scale。K-C量化场景下，数据类型支持`torch.float32`，维度为1D，shape为\(H2\)，数据格式支持$ND$。mx量化场景下，数据类型支持`torch_npu.float8_e8m0`，维度为3D，shape为\(H2, ceil\(H1/64\), 2\)，数据格式支持$ND$。
...
- **x1\_quant\_mode**（`int`）：**可选参数**，表示左矩阵的量化方式。当前版本支持配置为3和6，分别表示pertoken量化和mx量化。
- **x2\_quant\_mode**（`int`）：**可选参数**，表示右矩阵的量化方式。当前版本支持配置为2和6，表示perchannel量化和mx量化。
...
- **all2all\_axes**（`List[int]`）：**可选参数**，AlltoAll和Permute数据交换的方向，支持为空或者\[-1, -2\]，表示将Matmul结果由\(BS, H2\)转为\(BS\*rankSize, H2/rankSize\)。
...
- **output\_scale\_dtype**（`int`）：**可选参数**，代表输出量化系数的数据类型。预留参数，当前版本传默认值即可。
...
- 参数里Shape使用的变量如下：
  - BS：输入左矩阵的第一维度大小，表示输入序列sequence的条数，BS\*rankSize取值范围为\[1, 2147483647\]。
...
- 当前`x1_quant_mode`和`x2_quant_mode`的组合仅支持\[3, 2\]和\[6, 6\]，分别表示K-C量化和mx量化。
- mx量化场景下，`x2`必须转置（即需要使用.t\(\)方法），转置后的shape需满足\(H1, H2\)。
- mx量化场景下，且`x1`和`x2`的输入为`torch_npu.float4_e2m1fn_x2`时，需要满足ceil\(H1/32\)为偶数，且H1为偶数。
- 当前场景`x1_scale`和`x2_scale`必须传入，不允许为None。
...
- 单算子模式调用
    ...
    def run_quant_matmul_all_to_all(rank, world_size, master_ip, master_port, x1_shape, x2_shape, bias_shape, x1_scale_shape, x2_scale_shape):
        ...
        x1_np = np.random.uniform(-1, 1, x1_shape).astype(float8_e4m3fn)
        x1_tensor = torch.from_numpy(x1_np.astype(np.float32)).to(torch.float8_e4m3fn).npu()
        ...
        x1_scale_tensor = torch.randn(x1_scale_shape, dtype=torch.float32).npu()
        x2_scale_tensor = torch.randn(x2_scale_shape, dtype=torch.float32).npu()
        output = torch_npu.npu_quant_matmul_all_to_all(
            x1_tensor,
            x2_tensor,
            hcom_info,
            world_size,
    ...
    if __name__ == "__main__":
        ...
        mp.spawn(
            run_quant_matmul_all_to_all,
            args=(worksize, master_ip, master_port, x1_shape, x2_shape, bias_shape, x1_scale_shape, x2_scale_shape),
            nprocs=worksize,
        )
    ```
...
- 图模式调用
    ...
    class Model(torch.nn.Module):
        def __init__(self):
            super().__init__()
        def forward(self, x1, x2, hcom, world_size, bias, x1_scale, x2_scale):
            return torch_npu.npu_quant_matmul_all_to_all(x1=x1, x2=x2, hcom=hcom, world_size=world_size,
    ...
    def create_tensor(tensor_flag, shape=(), dtype="", transpose=False):
        ...
        if tensor_flag:
            ...
            tensor = None
        return tensor
    def run_quant_matmul_allto_all_graph(rank, world_size, master_ip, master_port, x1_shape, x2_shape, bias_shape, x1_scale_shape, x2_scale_shape):
        # get hcom_info
        torch_npu.npu.set_device(rank)
        init_method = "tcp://" + master_ip + ":" + master_port
        dist.init_process_group(backend="hccl", rank=rank, world_size=world_size, init_method=init_method)
        ...
        x1_tensor = create_tensor(1, [1024, 256], "float8_e4m3fn")
        x2_tensor = create_tensor(1, [256, 3072], "float8_e4m3fn")
        bias_tensor = torch.randn(bias_shape, dtype=torch.float32).npu()
        x1_scale_tensor = torch.randn(x1_scale_shape, dtype=torch.float32).npu()
        x2_scale_tensor = torch.randn(x2_scale_shape, dtype=torch.float32).npu()
        cpu_model = Model()
    ...
    if __name__ == "__main__":
        ...
        x2_scale_shape = [3072]
        mp.spawn(
            run_quant_matmul_allto_all_graph,
            args=(world_size, master_ip, master_port, x1_shape, x2_shape, bias_shape, x1_scale_shape, x2_scale_shape),
            nprocs=world_size,
        )
    ```
