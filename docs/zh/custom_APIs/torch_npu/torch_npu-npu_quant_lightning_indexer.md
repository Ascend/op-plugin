...
- **query\_dtype**（`int`）：可选参数，表示query实际传入的数据类型，支持`int8`。

- **key\_dtype**（`int`）：可选参数，表示key实际传入的数据类型，支持`int8`。
