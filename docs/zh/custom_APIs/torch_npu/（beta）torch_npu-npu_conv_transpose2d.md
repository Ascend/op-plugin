...
- **input**（`Tensor`）：必选参数，输入张量，其shape为(minibatch, in_channels, iH, iW)。
- **weight**（`Tensor`）：必选参数，过滤器张量，其shape为(in_channels, out_channels/groups, kH, kW)。
- **bias**（`Tensor`）：可选参数，默认为None，偏置张量，其shape为(out_channels)。
