...
将`int32`类型的输入`tensor`打包为`int4`存放，每8个`int4`数据通过一个`int32`数据承载，并进行交错排放。
...
- **weight** (`Tensor`)：输入的weight，数据格式支持$ND$、$FRACTAL\_NZ$，数据类型支持`int32`，不支持非连续的`Tensor`；维度支持2维，shape支持$(k, n)$、 $(n, k)$，最后一维的长度需为8的整数倍，元素的值需要在`int4`的表示范围内，即[-8, 7]。
