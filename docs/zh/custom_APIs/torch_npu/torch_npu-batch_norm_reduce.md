...
- **input** (`Tensor`)：必选参数，表示待归约的输入Tensor。支持4D Tensor，shape为 `(N, C, H, W)` 。数据类型支持 `float32`、`float16`、`bfloat16`。
- **eps** (`float`)：必选参数，保留参数。该参数不参与 `sum` 和 `square_sum` 的计算，建议按BatchNorm常用配置传入 `1e-5`。

## 返回值说明

- **sum** (`Tensor`)：输入Tensor按通道维归约后的元素和，为1D `float32` Tensor，长度为通道维长度。输入为4D Tensor且shape为 `(N, C, H, W)` 时，输出shape为 `(C,)` 。
- **square_sum** (`Tensor`)：输入Tensor按通道维归约后的平方和，为1D `float32` Tensor，shape同 `sum` 。
