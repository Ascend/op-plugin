...
-   **nmsed_boxes** (`Tensor`)：shape为\(batch, max\_total\_size, 4\)的3D张量，指定每批次输出的nms框，数据类型为`float16`。
-   **nmsed_scores** (`Tensor`)：shape为\(batch, max\_total\_size\)的2D张量，指定每批次输出的nms分数，数据类型为`float16`。
-   **nmsed_classes**(`Tensor`)：shape为\(batch, max\_total\_size\)的2D张量，指定每批次输出的nms类，数据类型为`float16`。
