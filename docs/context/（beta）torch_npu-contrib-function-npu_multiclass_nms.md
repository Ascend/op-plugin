...
- **score_thr** (`float`)：bbox阈值，分数低于它的bbox将不被考虑。默认值为0.05。
- **nms_thr** (`float`)：NMS IoU阈值。最初的实现是传递\{"iouthreshold": 0.45\}字典，这里做了简化。默认值为0.45。
- **max_num** (`int`)：如果NMS后的bbox数超过max_num值，则只保留最大max_num；如果NMS后的bbox数小于max_num值，则输出将零填充到max_num值。在NPU上需提前申请内存，因此目前不能将max_num值设置为-1。默认值为50。
- **score_factors** (`Tensor`)：NMS应用前用来乘分数的因子。默认值为None。

## 返回值说明
`Tuple`

(bboxes, labels)，shape为(k, 5)和(k, 1)的张量。标签以0为基础。
