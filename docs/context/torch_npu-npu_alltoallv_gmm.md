...
- 算子功能：MoE网络中，完成路由专家AlltoAllv、Permute、GroupedMatMul融合并实现与共享专家MatMul并行融合，先通信后计算。
...
-   **gmm\_y**（Tensor）：计算输出，表示最终的计算结果，数据类型与输入gmm\_x保持一致，支持2维，shape为\(A, N1\)。
-   **mm\_y**（Tensor）：计算输出，共享专家MatMul的输出，数据类型与mm\_x保持一致，支持2维，shape为\(BS, N2\)。仅当传入mm\_x与mm\_weight才输出。
-   **permute\_out**（Tensor）：计算输出，Permute之后的输出，数据类型与gmm\_x保持一致。
