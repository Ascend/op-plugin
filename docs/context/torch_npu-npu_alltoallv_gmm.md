...
-   **send\_counts**（`int[]`）：必选参数，表示发送给其他卡的token数，数据类型支持int，取值大小为e\*`ep_world_size`，最大为256。
-   **recv\_counts**（`int[]`）：必选参数，表示接收其他卡的token数，数据类型支持int，取值大小为e\*`ep_world_size`，最大为256。
-   **send\_counts\_tensor**（`Tensor`）：可选参数，数据类型支持int，shape为\(e\*`ep_world_size`,\)，数据格式支持ND。**当前版本暂不支持**，使用默认值即可。
-   **recv\_counts\_tensor**（`Tensor`）：可选参数，数据类型支持int，shape为\(e\*`ep_world_size`,\)，数据格式支持ND。**当前版本暂不支持**，使用默认值即可。
