...
-   计算公式：
    1.  对输入expert_idx做排序，得出排序的结果sorted_expert_idx和对应的序号sorted_row_idx：
    ...
    5.  对quantResult取前NUM_ROWS个sorted_row_idx的对应位置的值，得出输出expanded_x：

        ![](./figures/zh-cn_formulaimage_0000002320465592.png)

    6.  输出参数expanded_row_idx的有效元素数量available_idx_num计算方式为，expert_idx中active_expert_range范围内的元素的个数：
