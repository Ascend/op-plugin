...
- **`pertoken_scale`** (`bool`): Specifies whether to include `pertoken_scale` parameters in the computation. If set to `False`, `pertoken_scale` is excluded from the quantized matrix multiplication computation. Currently, this parameter is not supported on Atlas inference products.
...
- **`pertoken_scale`** (`Tensor`): Optional. Per-token scale value for quantized computation. The data type can be `float32`. The data layout can be ND. The parameter must be 1D with shape `(m,)`, where `m` matches the `m` dimension of `x1`. The parameter can be non-empty only when the output data type is `float16` or `bfloat16`. This parameter is currently not supported on Atlas inference products.
