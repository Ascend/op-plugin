...
Currently, Atlas inference products support only `perchannel` quantization.
...
- **`antiquant_group_size`** (`int`): Group size for `pergroup` scenarios. The default value is `0`. The value must be divisible by 32 in the range of `[32, K-1]`. Atlas inference products: This parameter is not supported currently.
