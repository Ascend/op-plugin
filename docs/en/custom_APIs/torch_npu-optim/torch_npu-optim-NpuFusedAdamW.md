...
Provides a high-performance Adam with decoupled weight decay (AdamW) optimizer implemented through tensor fusion. This optimizer is an improved version of Adam (Adaptive Moment Estimation). It decouples the weight decay term from gradient computation and applies it directly to the parameter update step, addressing the interference between L2 regularization and adaptive learning rates in the original Adam algorithm. Its core functionality is compatible with `torch.optim.AdamW`.
...
- **`betas`** (`Tuple[float, float]`): Optional. Coefficients used to compute the running averages of gradients and squared gradients. This parameter must be a tuple containing two values. The default value is `(0.9, 0.999)`. A `ValueError` will be raised if any value in `betas` is less than `0` or greater than or equal to `1`.
- **`eps`** (`float`): Optional. Value added to the denominator to prevent division by zero and improve numerical stability. The default value is `1e-8`. A `ValueError` will be raised if `eps` is less than `0`.
