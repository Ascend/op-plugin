...
- **`eps`** (`float`): Optional. Value added to the denominator to prevent division by zero and improve numerical stability. The default value is `1e-10`. A `ValueError` will be raised if `eps` is less than `0`.
...
`NpuFusedRMSpropTF` works normally when performing in-place computations on model parameter objects or reading parameter values.
