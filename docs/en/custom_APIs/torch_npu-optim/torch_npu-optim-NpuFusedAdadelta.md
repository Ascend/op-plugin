...
- **`rho`** (`float`): Optional. Decay coefficient for the running average of squared gradients (default value: `0.9`). A `ValueError` will be raised if `rho` is less than `0` or greater than `1`.
- **`eps`** (`float`): Optional. Value added to the denominator to prevent division by zero and improve numerical stability (default value: `1e-6`). A `ValueError` will be raised if `eps` is less than `0`.
