...
- **`betas`** (`Tuple[float, float]`): Optional. Coefficients used to compute the running averages of gradients and squared gradients. This parameter must be a tuple containing two values. The default value is `(0.9, 0.999)`.
- **`eps`** (`float`): Optional. Value added to the denominator to prevent division by zero and improve numerical stability. The default value is `1e-8`.
