...
- **`nesterov`** (`bool`): Optional. Specifies whether to use Nesterov momentum. The default value is `False`. A `ValueError` will be raised if this parameter is set to `True` while `momentum` is less than `0` or `dampening` is not equal to `0`.
