...
Provides a high-performance Adam optimizer implemented through tensor fusion. The core functionality is compatible with `torch.optim.Adam`.

For details about the functions and principles of Adam, see [Adam](https://pytorch.org/docs/stable/generated/torch.optim.Adam.html#adam).
...
- **`betas`** (`Tuple[float, float]`): Optional. Coefficients used to compute the running averages of gradients and squared gradients. This parameter must be a tuple containing two values (default value: `(0.9, 0.999)`). A `ValueError` will be raised if any value in `betas` is less than `0` or greater than or equal to `1`.
- **`eps`** (`float`): Optional. Value added to the denominator to prevent division by zero and improve numerical stability (default value: `1e-8`). A `ValueError` will be raised if `eps` is less than `0`.
...
- **`amsgrad`** (`bool`): Optional. Specifies whether to use the `AMSGrad` variant. The default value is `False`.

## Return Values

Object of the `NpuFusedAdam` type.

## Constraints

The implementation mechanism of `NpuFusedAdam` requires that each model parameter object inside `params` must not be reallocated during use. Otherwise, unpredictable results may occur. Operations that cause model parameter objects to be reallocated include, but are not limited to:
