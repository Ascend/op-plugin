...
- **`eps`** (`float`): Optional. Value added to the denominator to prevent division by zero and improve numerical stability. The default value is `1e-6`. A `ValueError` will be raised if `eps` is less than `0`.
...
- **`use_global_grad_norm`** (`bool`): Optional. Specifies whether to use the global gradient norm. The default value is `False`.
