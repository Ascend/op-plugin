...
- **`custom_trace_id_callback`** (`str`): Optional. Generates a `trace_id` to identify each set of Profiler data. The `trace_id` is output to the `profiler_metadata.json` file.
