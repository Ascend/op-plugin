...
- **`repeat`** (`int`): Optional. Number of times to repeat the `wait + warmup + active` process. The default value is `0`, indicating that the process is repeated until the training stops. You are advised to set this parameter to an integer greater than `0`.
