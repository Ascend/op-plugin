...
- **`custom_trace_id_callback`** (`Callable`): Optional. Generates a `trace_id` to identify each set of profiler data. The `trace_id` is output to the `profiler_metadata.json` file.
...
The profile data collected by `torch_npu.profiler.profile` is automatically parsed and saved to the directory specified by `torch_npu.profiler.tensorboard_trace_handler`. Please refer to [MindStudio Insight System Tuning](https://gitcode.com/Ascend/msinsight/blob/26.1.0/docs/en/user_guide/system_tuning.md) for data visualization and analysis.

- Basic example for collecting profile data
  ...
  # Add basic configuration parameters for profile data collection. For details about the parameters, see the parameter description below.
  ...
  with torch_npu.profiler.profile(
      ...
      schedule=torch_npu.profiler.schedule(wait=0, warmup=0, active=1, repeat=1, skip_first=0, skip_first_wait=0),    # Used together with prof.step()
...
- Generate a trace_id to identify profiler data
  ...
  # Custom trace_id generator
  class RepeatTraceIdGenerator:
      def __init__(self):
          self.repeat_count = 0    # Start counting from 0

      def __call__(self) -> str:
          # Increment the count each time profiling is started
          current_id = self.repeat_count
          self.repeat_count += 1
          return str(current_id)

  # Create a trace_id generator
  ...
  # Add basic configuration parameters for profile data collection
  with torch_npu.profiler.profile(
      activities=[
          torch_npu.profiler.ProfilerActivity.CPU,
          torch_npu.profiler.ProfilerActivity.NPU
