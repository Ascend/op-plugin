...
- **`op_type`** (`std::vector<std::string>&`): Required. Operation sequence. Each element in the vector is either `isend` or `irecv`, specifying the communication operation type for the corresponding tensor.
