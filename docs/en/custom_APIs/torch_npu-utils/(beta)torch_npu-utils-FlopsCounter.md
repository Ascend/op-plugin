...
The following describes the internal state parameters of this class. Users can modify these parameters through member functions.
...
- **`torch_npu.utils.FlopsCounter.stop()`**
    ...
    Disables FLOPs tracking. `FlopsCounter.stop()` sets the enable flag (`isEnabled_`) and pause flag (`isPaused_`) to `False`, disabling FLOPs computation. FLOPs with recomputation (`traversedCount`) and FLOPs without recomputation (`recordedCount`) are not tracked. The values of `traversedCount` and `recordedCount` are also reset to `0`.
