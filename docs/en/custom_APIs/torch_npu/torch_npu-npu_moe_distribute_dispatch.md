...
- **`x`** (`Tensor`): Token data used for computation and dispatched to other ranks based on `expert_ids`. This parameter must be 2D with shape `(BS, H)`, indicating that there are `BS` (batch size) tokens. The data type can be `torch.bfloat16` or `torch.float16`. The data layout is ND. Non-contiguous tensors are supported.
...
- **`group_ep`** (`str`): EP communication domain name used for expert parallelism. The string length range is [1, 128). On Atlas A3 training products/Atlas A3 inference products, the value of this parameter must differ from `group_tp`.
- **`ep_world_size`** (`int`): Size of the EP communication domain.
    - Atlas A2 training products/Atlas A2 inference products: Valid values are `16`, `32`, or `64`.
    - Atlas A3 training products/Atlas A3 inference products: Valid values are `8`, `16`, `32`, `64`, `128`, `144`, `256`, or `288`.

- **`ep_rank_id`** (`int`): Rank ID of the current rank within the EP communication domain. The value range is [0, `ep_world_size`). The `ep_rank_id` values of all ranks within the same EP communication domain must be unique.
- **`moe_expert_num`** (`int`): Number of MoE experts. The value range is [1, 512], and the condition `moe_expert_num % (ep_world_size - shared_expert_rank_num) == 0` must be satisfied.
    - Atlas A2 training products/Atlas A2 inference products: The following condition must also be met: `moe_expert_num/(ep_world_size - shared_expert_rank_num) <= 24`.
...
- **`expert_scales`** (`Tensor`): Optional. Top-K expert weights for each token.
    - Atlas A2 training products/Atlas A2 inference products: This parameter must be 2D with shape `(BS, K)`. The data type can be `torch.float`. The data layout is ND. Non-contiguous tensors are supported.
...
- **`tp_world_size`** (`int`): Optional. Size of the TP communication domain. When TP domain communication is not involved, use the default value `0`.
...
- **`shared_expert_num`** (`int`): Number of shared experts, where a shared expert can be replicated and deployed across multiple ranks.
    - Atlas A2 training products/Atlas A2 inference products: Currently, this parameter is not supported. Retain the default value.
    - Atlas A3 training products/Atlas A3 inference products: Only the default value `1` is supported.

- **`shared_expert_rank_num`** (`int`): Optional. Number of shared-expert ranks.
    - Atlas A2 training products/Atlas A2 inference products: Shared experts are not supported. Set it to `0`.
    - Atlas A3 training products/Atlas A3 inference products: The value range is [0, `ep_world_size`). The value `0` indicates that no shared experts are used. If the value is not `0`, the condition `ep_world_size % shared_expert_rank_num == 0` must be satisfied.
...
- **`dynamic_scales`** (`Tensor`): Dynamically quantized parameters obtained through computation. This output is available only when `quant_mode` is not `0`. This parameter must be 1D with shape `(A,)`. The data type can be `torch.float`. The data layout can be ND. Non-contiguous tensors are supported.
...
- **`expand_scales`** (`Tensor`): Output after the alltoallv operation is performed on `expert_scales` and `x`.
    - Atlas A2 training products/Atlas A2 inference products: This parameter must be 1D with shape `(A,)`. The data type can be `torch.float`. The data layout must be ND. Non-contiguous tensors are supported.
    - Atlas A3 training products/Atlas A3 inference products: This output is not supported. `None` is returned.
...
- Variables used in parameter tensor shapes:
    ...
    - `H`: Hidden layer size.
        - Atlas A2 training products/Atlas A2 inference products: The value range is (0, 7168], and the value must be an integer multiple of 32.
        - Atlas A3 training products/Atlas A3 inference products: Only the value `7168` is supported.

    - `BS`: Number of tokens to be sent.
        - Atlas A2 training products/Atlas A2 inference products: The value range is 0 < `BS` <= 256.
    ...
    - `K`: Number of top-K experts selected. The value range is 0 < `K` ≤ `moe_expert_num`.
        - Atlas A2 training products/Atlas A2 inference products: The value range is 0 < `K` <= 16.
        - Atlas A3 training products/Atlas A3 inference products: The value range is 0 < `K` <= 8.

    - `server_num`: Number of server nodes. In single-server scenarios, the value can be `1`; in multi-server scenarios, the value can only be `2`, `4`, or `8`.
...
- Single-operator call
    ...
    server_num = 1                       # Single-server scenario
    server_index = 0
    port = 50001
    master_ip = '127.0.0.1'
    dev_num = 16
