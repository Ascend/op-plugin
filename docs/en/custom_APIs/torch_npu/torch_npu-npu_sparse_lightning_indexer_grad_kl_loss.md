...
- Implements the backward computation of `npu_lightning_indexer` and integrates the computation of loss. `npu_lightning_indexer` selects the top-k items with the highest intrinsic connection between `query` or `key` for attention. The items are stored in `sparse_indices`. This reduces the attention computation workload in long-sequence scenarios and improves training performance.
