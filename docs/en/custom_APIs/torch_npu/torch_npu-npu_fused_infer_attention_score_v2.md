...
- **`actual_seq_kvlen`** (`List[int]`): Optional. Valid sequence length of `key` and `value` across different batches. The data type can be `int64`. The default value is `None`, indicating that the value is identical to the sequence length `S` in the `key` and `value` shapes. Constraints vary across different `Q_S` values. For details, see [Constraints for Q_S>1](#en-us_topic_0000001832267082_section_qs_gt1_constraint) and [Constraints for Q_S=1](#en-us_topic_0000001832267082_section_qs_eq1_constraint).
- **`block_table`** (`Tensor`): Optional. Block mapping table used for KV storage in PagedAttention. The data type can be `int32`. The data layout can be ND. This parameter can be set to `None` if this feature is not used.
...
- **`sparse_mode`** (`int`): Optional. Sparse mode. The default value is `0`. The data type can be `int64`. This parameter is invalid when `Q_S` is `1` and no RoPE input is provided. For details about the overall constraints when `input_layout` is `TND`, `TND_NTD`, or `NTD_TND`, see [MLA Constraints](#en-us_topic_0000001832267082_section_mla_constraint). For details about the overall constraints in GQA fake-quantization scenarios, see [GQA Fake Quantization + KV NZ Layout Constraints](#en-us_topic_0000001832267082_section_gqa_nz_constraint). Currently, only the values 0, 1, 2, 3, 4, and 9 are supported. The values 5, 6, 7, and 8 (representing `prefix`, `global`, `dilated`, and `block_local`, respectively) are not yet implemented and should not be used.
    ...
    | 9 | treeMask | Tree attention mask for speculative decoding scenarios. Supported only in MLA scenarios (where `query_rope` and `key_rope` are not `None`). Left padding, `pse_shift`, and `sharedPrefix` are not supported. The output `dtype` does not support `int8`. Each batch must satisfy $Q\_S \le KV\_S$. | Required (custom tree mask) |
    
- **`block_size`** (`int`): Optional. Maximum number of tokens in each block of KV storage in PagedAttention. The data type can be `int64`. The default value is 0.
...
- **`out_dtype`** (`int`): Optional. Output data type. When the input is `int8` or `float8_e4m3fn`, this parameter can be used to specify the output data type (such as `float8_e5m2`). This parameter can be set to `None` if this feature is not used.

## Return Values<a name="en-us_topic_0000001832267082_section22231435517"></a>

- **`attention_out`** (`Tensor`): Output tensor in the formula. The data type can be `float16`, `bfloat16`, or `int8`. The data layout can be ND. Restriction: The `D` dimension of the output must be identical to that of `value`, and all other dimensions must match those of the input parameter `query`.
...
>
> - [**General Constraints**](#en-us_topic_0000001832267082_section_general_constraint): Handling of empty inputs, consistency between `key`/`value` shapes, and `int8` quantization limitations.
...
- **When `Q_S` is greater than 1**:
    ...
    - PagedAttention scenarios:
        - PagedAttention can be enabled only when `block_table` exists and is valid, and `key` and `value` are arranged in contiguous memory based on the indices in `block_table`. The supported data types for `key` and `value` are `float16` and `bfloat16`. In this case, `input_layout` of `key` and `value` is ignored. The `block_table` contains block IDs. Their validity is not verified and must be ensured by the user.
        ...
        - In PagedAttention scenarios, two layout formats are supported, and the supported data types are `float32` and `bfloat16`. Cases where `query` is `int8` are not supported.
...
- **When `Q_S` is equal to 1**:
    ...
    - PagedAttention scenarios:
        ...
        - In PagedAttention scenarios, the product of all dimensions of the tensors corresponding to `key` and `value` must not exceed the representable range of `int32`.
