...
- **`sparse_mode`** (`int`): Optional. Sparse mode. The default value is `0`. The data type can be `int64`. This parameter is invalid when `Q_S` is `1` and no RoPE input is provided. For details about the overall constraints when `input_layout` is `TND`, `TND_NTD`, or `NTD_TND`, see [MLA Constraints](#en-us_topic_0000001832267082_section_mla_constraint). For details about the overal constraints in GQA fake-quantization scenarios, see [GQA Fake Quantization + KV NZ Layout Constraints](#en-us_topic_0000001832267082_section_gqa_nz_constraint). Currently, only the values 0, 1, 2, 3, 4, and 9 are supported. The values 5, 6, 7, and 8 (representing `prefix`, `global`, `dilated`, and `block_local`, respectively) are not yet implemented and should not be used.
