...
- **`self`** (`Tensor`): Required. Input box tensor containing the batch size. The data type must be `torch.float16`. Example shape is `(batch_size, num_anchors, q, 4)`, where `q` is `1` or `num_classes`.
- **`scores`** (`Tensor`): Required. Input score tensor. The data type is `torch.float16`. Example shape is `(batch_size, num_anchors, num_classes)`.
- **`score_threshold`** (`float`): Required. Score filter threshold used to screen boxes and remove boxes with low scores. The data type is `torch.float32`.
- **`iou_threshold`** (`float`): Required. NMS IoU threshold used to set a threshold and remove boxes higher than the threshold. The data type is `torch.float32`.
