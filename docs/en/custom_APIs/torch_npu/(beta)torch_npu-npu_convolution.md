...
- **`weight`** (`Tensor`): Filter tensor with shape `(out_channels, in_channels/groups, kH, kW)` or `(out_channels, in_channels/groups, kT, kH, kW)`.
- **`bias`** (`Tensor`): Bias tensor with shape `(out_channels)`. This parameter can be `None`.
