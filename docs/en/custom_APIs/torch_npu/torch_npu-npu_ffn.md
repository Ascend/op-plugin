...
- Provides Mixture-of-Experts Feed-Forward Network (MoeFFN) and Feed-Forward Network (FFN) computation features. FFN is used when there are no expert groups (`expert_tokens` is empty) and MoeFFN is used when there are expert groups (`expert_tokens` is not empty).
- Formula:
...
> [!NOTE]  
> When the activation function is `geglu`, `swiglu`, or `reglu`, performance optimization using the FFN fusion operator must meet the following threshold requirements. The FFN fusion operator may be enabled only if, among the small operators corresponding to the FFN structure across the entire network, the use cases with a vector execution time of at least 30 μs account for more than 10% of the total time. Alternatively, if the performance of these small operators is unknown, you can try enabling FFN fusion. If performance deteriorates, disable it.
