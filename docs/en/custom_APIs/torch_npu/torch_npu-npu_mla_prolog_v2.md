...
- **`weight_uq_qr`** (`Tensor`): Required. Combined upsampling weight matrix and position embedding weight matrix for query computation, W<sup>UQ</sup> and W<sup>QR</sup> in the formulas. This parameter must be 2D with shape `(Hcq, N * (D + Dr))`. The data type can be `bfloat16` or `int8`. The data layout can be FRACTAL\_NZ.
...
- **`weight_dkv_kr`** (`Tensor`): Required. Combined downsampling weight matrix and position embedding weight matrix for key computation, W<sup>DKV</sup> and W<sup>KR</sup> in the formulas. This parameter must be 2D with shape `(He, Hckv + Dr)`. The data type can be `bfloat16` or `int8`. The data layout can be FRACTAL\_NZ.
...
- **`rope_sin`** (`Tensor`): Required. Sine parameter matrix for rotary position embedding (`ROPE`) computation. This parameter must be 2D or 3D, with shape `(T, Dr)` or `(B, S, Dr)`. The data type can be `bfloat16`. The data layout can be ND.
- **`rope_cos`** (`Tensor`): Required. Cosine parameter matrix for rotary position embedding (`ROPE`) computation. This parameter must be 2D or 3D, with shape `(T, Dr)` or `(B, S, Dr)`. The data type can be `bfloat16`. The data layout can be ND.
- **`cache_index`** (`Tensor`): Required. Paged linear slot indices for writing to `kv_cache` and `kr_cache`. This parameter must be 1D or 2D, with shape `[T]` or `[B, S]`. This parameter is of type `int64`. The data layout can be `ND`. Each value must fall within the range `[0, L)`, where **`L`** represents the product of the sizes of the 0th and 1st dimensions of `kv_cache`, which must be identical to the product of the sizes of the 0th and 1st dimensions of `kr_cache`.
...
- **`kr_cache`** (`Tensor`): Required. Cache tensor for key rotary position embedding. This parameter must be 4D with shape `(BlockNum, BlockSize, Nkv, Dr)`. The data type can be `bfloat16` or `int8`. The data layout can be ND.
...
- **`dequant_scale_w_dq`** (`Tensor`): Optional. Parameter used for dequantization after downsampling when the input `weight_dq` is of type `int8`. The quantization mode of `weight_dq` is per-channel. This parameter must be 2D with shape `[1, Hcq]`. This parameter is of type `float`. The data layout can be `ND`.
...
- **`dequant_scale_w_dkv_kr`** (`Tensor`): Optional. Parameter used for dequantization after matrix multiplication when the input `weight_dkv_kr` is of type `int8`. The quantization mode of `weight_dkv_kr` is per-channel. This parameter must be 2D with shape `[1, Hckv + Dr]`. This parameter is of type `float`. The data layout can be `ND`.
...
- **`query_rope`** (`Tensor`): Output tensor for query position embedding, q<sup>R</sup> in the formulas. This parameter must be 3D or 4D with shape `(T, N, Dr)` or `(B, S, N, Dr)`. The data type can be `bfloat16`. The data layout can be ND.
