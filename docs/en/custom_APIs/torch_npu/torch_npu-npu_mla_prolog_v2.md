...
- Performs computation during Multi-Head Latent Attention (MLA) preprocessing in inference scenarios. The main computation flow consists of five separate paths.
    ...
    - In the second path, the tensor is multiplied by W<sup>QR</sup> and processed through rotary position embedding (`ROPE`) to yield q<sup>R</sup>.
...
- Formulas:
    ...
    - Query `ROPE` rotary position embedding formula:
    ...
    - Key `ROPE` rotary position embedding formula:
...
- **`token_x`** (`Tensor`): Required. `x` in the formulas. This parameter must be 2D or 3D, with shape `(T, He)` or `(B, S, He)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be ND.
- **`weight_dq`** (`Tensor`): Required. Downsampling weight matrix for query computation, W<sup>DQ</sup> in the formulas. This parameter must be 2D with shape `(He, Hcq)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be `FRACTAL_NZ`. The layout can be converted from `ND` by using `torch_npu.npu_format_cast`.
- **`weight_uq_qr`** (`Tensor`): Required. Combined upsampling weight matrix and position embedding weight matrix for query computation, W<sup>UQ</sup> and W<sup>QR</sup> in the formulas. This parameter must be 2D with shape `(Hcq, N * (D + Dr))`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be FRACTAL\_NZ.
- **`weight_uk`** (`Tensor`): Required. Upsampling weight matrix for key computation, corresponding to W<sup>UK</sup> in the formulas. This parameter must be 3D with shape `(N, D, Hckv)`. The data type can be `torch.bfloat16`. The data layout can be ND.
- **`weight_dkv_kr`** (`Tensor`): Required. Combined downsampling weight matrix and position embedding weight matrix for key computation, W<sup>DKV</sup> and W<sup>KR</sup> in the formulas. This parameter must be 2D with shape `(He, Hckv + Dr)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be FRACTAL\_NZ.
- **`rmsnorm_gamma_cq`** (`Tensor`): Required. The $\gamma$ parameter in the `RmsNorm` formula for computing c<sup>Q</sup>. This parameter must be 1D with shape `(Hcq,)`. The data type can be `torch.bfloat16`. The data layout can be ND.
- **`rmsnorm_gamma_ckv`** (`Tensor`): Required. The $\gamma$ parameter in the `RmsNorm` formula for computing c<sup>KV</sup>. This parameter must be 1D with shape `(Hckv,)`. The data type can be `torch.bfloat16`. The data layout can be ND.
- **`rope_sin`** (`Tensor`): Required. Sine parameter matrix for rotary position embedding (`ROPE`) computation. This parameter must be 2D or 3D, with shape `(T, Dr)` or `(B, S, Dr)`. The data type can be `torch.bfloat16`. The data layout can be ND.
- **`rope_cos`** (`Tensor`): Required. Cosine parameter matrix for rotary position embedding (`ROPE`) computation. This parameter must be 2D or 3D, with shape `(T, Dr)` or `(B, S, Dr)`. The data type can be `torch.bfloat16`. The data layout can be ND.
- **`cache_index`** (`Tensor`): Required. Paged linear slot indices for writing to `kv_cache` and `kr_cache`. This parameter must be 1D or 2D, with shape `[T]` or `[B, S]`. This parameter is of type `torch.int64`. The data layout can be `ND`. Each value must fall within the range `[0, L)`, where **`L`** represents the product of the sizes of the 0th and 1st dimensions of `kv_cache`, which must be identical to the product of the sizes of the 0th and 1st dimensions of `kr_cache`.
- **`kv_cache`** (`Tensor`): Required. Cache tensor for key and value indexing. This parameter must be 4D with shape `(BlockNum, BlockSize, Nkv, Hckv)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be ND.
- **`kr_cache`** (`Tensor`): Required. Cache tensor for key rotary position embedding. This parameter must be 4D with shape `(BlockNum, BlockSize, Nkv, Dr)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be ND.
...
- **`dequant_scale_x`** (`Tensor`): Optional. Dequantization scale parameter used after downsampling when the data type of input `token_x` is `torch.int8`. The quantization mode for `token_x` is `pertoken`. This parameter must be 2D with shape `(T, 1)` or `(BS, 1)`. The data type can be `torch.float`. The data layout can be ND.
- **`dequant_scale_w_dq`** (`Tensor`): Optional. Parameter used for dequantization after downsampling when the input `weight_dq` is of type `torch.int8`. The quantization mode of `weight_dq` is per-channel. This parameter must be 2D with shape `[1, Hcq]`. This parameter is of type `torch.float`. The data layout can be `ND`.
- **`dequant_scale_w_uq_qr`** (`Tensor`): Optional. Dequantization scale parameter used after the `MatmulQcQr` matrix multiplication operation. The quantization mode is `perchannel`. This parameter must be 2D with shape `(1, N * (D + Dr))`. The data type can be `torch.float`. The data layout can be ND.
- **`dequant_scale_w_dkv_kr`** (`Tensor`): Optional. Parameter used for dequantization after matrix multiplication when the input `weight_dkv_kr` is of type `torch.int8`. The quantization mode of `weight_dkv_kr` is per-channel. This parameter must be 2D with shape `[1, Hckv + Dr]`. This parameter is of type `torch.float`. The data layout can be `ND`.
- **`quant_scale_ckv`** (`Tensor`): Optional. Quantization scale parameter used when writing data to `kv_cache_out`. This parameter must be 2D with shape `(1, Hckv)`. The data type can be `torch.float`. The data layout can be ND.
- **`quant_scale_ckr`** (`Tensor`): Optional. Quantization scale parameter used when writing data to `kr_cache_out`. This parameter must be 2D with shape `(1, Dr)`. The data type can be `torch.float`. The data layout can be ND.
- **`smooth_scales_cq`** (`Tensor`): Optional. Scaling factors used for dynamic quantization of the `RmsNormCq` output. This parameter must be 2D with shape `(1, Hcq)`. The data type can be `torch.float`. The data layout can be ND.
...
- **`query`** (`Tensor`): Query output tensor, corresponding to q<sup>N</sup> in the formulas. This parameter must be 3D or 4D with shape `(T, N, Hckv)` or `(B, S, N, Hckv)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be ND.
- **`query_rope`** (`Tensor`): Output tensor for query position embedding, q<sup>R</sup> in the formulas. This parameter must be 3D or 4D with shape `(T, N, Dr)` or `(B, S, N, Dr)`. The data type can be `torch.bfloat16`. The data layout can be ND.
- **`kv_cache_out`** (`Tensor`): Tensor written to `kv_cache` through an in-place update, k<sup>C</sup> in the formulas. This parameter must be 4D with shape `(BlockNum, BlockSize, Nkv, Hckv)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be ND.
- **`kr_cache_out`** (`Tensor`): Tensor written to `kr_cache` through an in-place update, corresponding to k<sup>R</sup> in the formulas. This parameter must be 4D with shape `(BlockNum, BlockSize, Nkv, Dr)`. The data type can be `torch.bfloat16` or `torch.int8`. The data layout can be ND.
- **`dequant_scale_q_nope`** (`Tensor`): Dequantization scale parameter for the query output tensor. This parameter must be 1D or 3D. In full KV cache quantization scenarios, the shape is `(T, N, 1)` or `(B * S, N, 1)`. In other scenarios, the shape is `(1,)`. The data type can be `torch.float`. The data layout can be ND.
...
- Field definitions for shape formats:
    ...
    - `D`: dimension of query and key without position embedding. The value must be `128`.
    - `Dr`: dimension of query and key position embedding. The value must be `64`.
    - `Nkv`: attention head count for key and value. The value must be `1`.
    - `BlockNum`: number of blocks in PagedAttention scenarios, that is, the size of the 0-th dimension of `kv_cache` and `kr_cache`. Given a batch size of `B`, a KV sequence length of `Skv`, and a block size of `BlockSize`, the following condition should generally be satisfied: `BlockNum` ≥ ⌈`B` × `Skv` / `BlockSize`⌉ (`Skv` can be 0).
...
- In different quantization scenarios, the data types (dtypes) and shapes of parameters must satisfy the following conditions.
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1102141414313"><a name="en-us_topic_0000002313328922_p1102141414313"></a><a name="en-us_topic_0000002313328922_p1102141414313"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p8102514163119"><a name="en-us_topic_0000002313328922_p8102514163119"></a><a name="en-us_topic_0000002313328922_p8102514163119"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p17102314193112"><a name="en-us_topic_0000002313328922_p17102314193112"></a><a name="en-us_topic_0000002313328922_p17102314193112"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1210291473118"><a name="en-us_topic_0000002313328922_p1210291473118"></a><a name="en-us_topic_0000002313328922_p1210291473118"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p010219141312"><a name="en-us_topic_0000002313328922_p010219141312"></a><a name="en-us_topic_0000002313328922_p010219141312"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p31032147310"><a name="en-us_topic_0000002313328922_p31032147310"></a><a name="en-us_topic_0000002313328922_p31032147310"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p110361423111"><a name="en-us_topic_0000002313328922_p110361423111"></a><a name="en-us_topic_0000002313328922_p110361423111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p161031714153111"><a name="en-us_topic_0000002313328922_p161031714153111"></a><a name="en-us_topic_0000002313328922_p161031714153111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p201031114103115"><a name="en-us_topic_0000002313328922_p201031114103115"></a><a name="en-us_topic_0000002313328922_p201031114103115"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p51032014193110"><a name="en-us_topic_0000002313328922_p51032014193110"></a><a name="en-us_topic_0000002313328922_p51032014193110"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p19103151418313"><a name="en-us_topic_0000002313328922_p19103151418313"></a><a name="en-us_topic_0000002313328922_p19103151418313"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p6103131473113"><a name="en-us_topic_0000002313328922_p6103131473113"></a><a name="en-us_topic_0000002313328922_p6103131473113"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p9104171413118"><a name="en-us_topic_0000002313328922_p9104171413118"></a><a name="en-us_topic_0000002313328922_p9104171413118"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p0104101413314"><a name="en-us_topic_0000002313328922_p0104101413314"></a><a name="en-us_topic_0000002313328922_p0104101413314"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p51041914123119"><a name="en-us_topic_0000002313328922_p51041914123119"></a><a name="en-us_topic_0000002313328922_p51041914123119"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p710491419315"><a name="en-us_topic_0000002313328922_p710491419315"></a><a name="en-us_topic_0000002313328922_p710491419315"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p41041414163116"><a name="en-us_topic_0000002313328922_p41041414163116"></a><a name="en-us_topic_0000002313328922_p41041414163116"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p91041114113114"><a name="en-us_topic_0000002313328922_p91041114113114"></a><a name="en-us_topic_0000002313328922_p91041114113114"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p121041145312"><a name="en-us_topic_0000002313328922_p121041145312"></a><a name="en-us_topic_0000002313328922_p121041145312"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p410441493120"><a name="en-us_topic_0000002313328922_p410441493120"></a><a name="en-us_topic_0000002313328922_p410441493120"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p13105014123115"><a name="en-us_topic_0000002313328922_p13105014123115"></a><a name="en-us_topic_0000002313328922_p13105014123115"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p161051514173116"><a name="en-us_topic_0000002313328922_p161051514173116"></a><a name="en-us_topic_0000002313328922_p161051514173116"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1910571416319"><a name="en-us_topic_0000002313328922_p1910571416319"></a><a name="en-us_topic_0000002313328922_p1910571416319"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p191051614143117"><a name="en-us_topic_0000002313328922_p191051614143117"></a><a name="en-us_topic_0000002313328922_p191051614143117"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p121058149314"><a name="en-us_topic_0000002313328922_p121058149314"></a><a name="en-us_topic_0000002313328922_p121058149314"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p201051914193111"><a name="en-us_topic_0000002313328922_p201051914193111"></a><a name="en-us_topic_0000002313328922_p201051914193111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p1910551410318"><a name="en-us_topic_0000002313328922_p1910551410318"></a><a name="en-us_topic_0000002313328922_p1910551410318"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p11106914183110"><a name="en-us_topic_0000002313328922_p11106914183110"></a><a name="en-us_topic_0000002313328922_p11106914183110"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p51061814193115"><a name="en-us_topic_0000002313328922_p51061814193115"></a><a name="en-us_topic_0000002313328922_p51061814193115"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p1110681414313"><a name="en-us_topic_0000002313328922_p1110681414313"></a><a name="en-us_topic_0000002313328922_p1110681414313"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1110631463118"><a name="en-us_topic_0000002313328922_p1110631463118"></a><a name="en-us_topic_0000002313328922_p1110631463118"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p71064146316"><a name="en-us_topic_0000002313328922_p71064146316"></a><a name="en-us_topic_0000002313328922_p71064146316"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p151061114153117"><a name="en-us_topic_0000002313328922_p151061114153117"></a><a name="en-us_topic_0000002313328922_p151061114153117"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p5106714193110"><a name="en-us_topic_0000002313328922_p5106714193110"></a><a name="en-us_topic_0000002313328922_p5106714193110"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p141063148312"><a name="en-us_topic_0000002313328922_p141063148312"></a><a name="en-us_topic_0000002313328922_p141063148312"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1810781411315"><a name="en-us_topic_0000002313328922_p1810781411315"></a><a name="en-us_topic_0000002313328922_p1810781411315"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p181071814133111"><a name="en-us_topic_0000002313328922_p181071814133111"></a><a name="en-us_topic_0000002313328922_p181071814133111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p610721403111"><a name="en-us_topic_0000002313328922_p610721403111"></a><a name="en-us_topic_0000002313328922_p610721403111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p7107201413315"><a name="en-us_topic_0000002313328922_p7107201413315"></a><a name="en-us_topic_0000002313328922_p7107201413315"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p1310701410316"><a name="en-us_topic_0000002313328922_p1310701410316"></a><a name="en-us_topic_0000002313328922_p1310701410316"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p510781414315"><a name="en-us_topic_0000002313328922_p510781414315"></a><a name="en-us_topic_0000002313328922_p510781414315"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p15107131419311"><a name="en-us_topic_0000002313328922_p15107131419311"></a><a name="en-us_topic_0000002313328922_p15107131419311"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p0108141463115"><a name="en-us_topic_0000002313328922_p0108141463115"></a><a name="en-us_topic_0000002313328922_p0108141463115"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p4108414123111"><a name="en-us_topic_0000002313328922_p4108414123111"></a><a name="en-us_topic_0000002313328922_p4108414123111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p151081314153116"><a name="en-us_topic_0000002313328922_p151081314153116"></a><a name="en-us_topic_0000002313328922_p151081314153116"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1510861463119"><a name="en-us_topic_0000002313328922_p1510861463119"></a><a name="en-us_topic_0000002313328922_p1510861463119"></a>`torch.int64`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p181081514133118"><a name="en-us_topic_0000002313328922_p181081514133118"></a><a name="en-us_topic_0000002313328922_p181081514133118"></a>`torch.int64`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p18108314143115"><a name="en-us_topic_0000002313328922_p18108314143115"></a><a name="en-us_topic_0000002313328922_p18108314143115"></a>`torch.int64`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p15109131433112"><a name="en-us_topic_0000002313328922_p15109131433112"></a><a name="en-us_topic_0000002313328922_p15109131433112"></a>`torch.int64`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p31091614153119"><a name="en-us_topic_0000002313328922_p31091614153119"></a><a name="en-us_topic_0000002313328922_p31091614153119"></a>`torch.int64`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1610919146315"><a name="en-us_topic_0000002313328922_p1610919146315"></a><a name="en-us_topic_0000002313328922_p1610919146315"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p510911413112"><a name="en-us_topic_0000002313328922_p510911413112"></a><a name="en-us_topic_0000002313328922_p510911413112"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1610921483112"><a name="en-us_topic_0000002313328922_p1610921483112"></a><a name="en-us_topic_0000002313328922_p1610921483112"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p61097141319"><a name="en-us_topic_0000002313328922_p61097141319"></a><a name="en-us_topic_0000002313328922_p61097141319"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p101091814193117"><a name="en-us_topic_0000002313328922_p101091814193117"></a><a name="en-us_topic_0000002313328922_p101091814193117"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p16109101411314"><a name="en-us_topic_0000002313328922_p16109101411314"></a><a name="en-us_topic_0000002313328922_p16109101411314"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p1311013144317"><a name="en-us_topic_0000002313328922_p1311013144317"></a><a name="en-us_topic_0000002313328922_p1311013144317"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p11110414103116"><a name="en-us_topic_0000002313328922_p11110414103116"></a><a name="en-us_topic_0000002313328922_p11110414103116"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p4110121414313"><a name="en-us_topic_0000002313328922_p4110121414313"></a><a name="en-us_topic_0000002313328922_p4110121414313"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p9110121463118"><a name="en-us_topic_0000002313328922_p9110121463118"></a><a name="en-us_topic_0000002313328922_p9110121463118"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p14111161416312"><a name="en-us_topic_0000002313328922_p14111161416312"></a><a name="en-us_topic_0000002313328922_p14111161416312"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p511141420316"><a name="en-us_topic_0000002313328922_p511141420316"></a><a name="en-us_topic_0000002313328922_p511141420316"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p9112111473120"><a name="en-us_topic_0000002313328922_p9112111473120"></a><a name="en-us_topic_0000002313328922_p9112111473120"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p011241416315"><a name="en-us_topic_0000002313328922_p011241416315"></a><a name="en-us_topic_0000002313328922_p011241416315"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p191131214143118"><a name="en-us_topic_0000002313328922_p191131214143118"></a><a name="en-us_topic_0000002313328922_p191131214143118"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p171131514113119"><a name="en-us_topic_0000002313328922_p171131514113119"></a><a name="en-us_topic_0000002313328922_p171131514113119"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p11139145317"><a name="en-us_topic_0000002313328922_p11139145317"></a><a name="en-us_topic_0000002313328922_p11139145317"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p16113314153120"><a name="en-us_topic_0000002313328922_p16113314153120"></a><a name="en-us_topic_0000002313328922_p16113314153120"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p18114131413113"><a name="en-us_topic_0000002313328922_p18114131413113"></a><a name="en-us_topic_0000002313328922_p18114131413113"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p91141614143115"><a name="en-us_topic_0000002313328922_p91141614143115"></a><a name="en-us_topic_0000002313328922_p91141614143115"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p2011441433113"><a name="en-us_topic_0000002313328922_p2011441433113"></a><a name="en-us_topic_0000002313328922_p2011441433113"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p161143149317"><a name="en-us_topic_0000002313328922_p161143149317"></a><a name="en-us_topic_0000002313328922_p161143149317"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p4115114113119"><a name="en-us_topic_0000002313328922_p4115114113119"></a><a name="en-us_topic_0000002313328922_p4115114113119"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p81152014153118"><a name="en-us_topic_0000002313328922_p81152014153118"></a><a name="en-us_topic_0000002313328922_p81152014153118"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p311514144313"><a name="en-us_topic_0000002313328922_p311514144313"></a><a name="en-us_topic_0000002313328922_p311514144313"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p0116191443116"><a name="en-us_topic_0000002313328922_p0116191443116"></a><a name="en-us_topic_0000002313328922_p0116191443116"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p19116614173110"><a name="en-us_topic_0000002313328922_p19116614173110"></a><a name="en-us_topic_0000002313328922_p19116614173110"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p0116171433116"><a name="en-us_topic_0000002313328922_p0116171433116"></a><a name="en-us_topic_0000002313328922_p0116171433116"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p11116191483116"><a name="en-us_topic_0000002313328922_p11116191483116"></a><a name="en-us_topic_0000002313328922_p11116191483116"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p31167143319"><a name="en-us_topic_0000002313328922_p31167143319"></a><a name="en-us_topic_0000002313328922_p31167143319"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p181162148319"><a name="en-us_topic_0000002313328922_p181162148319"></a><a name="en-us_topic_0000002313328922_p181162148319"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p611671419319"><a name="en-us_topic_0000002313328922_p611671419319"></a><a name="en-us_topic_0000002313328922_p611671419319"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p4117514153118"><a name="en-us_topic_0000002313328922_p4117514153118"></a><a name="en-us_topic_0000002313328922_p4117514153118"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p1511719141313"><a name="en-us_topic_0000002313328922_p1511719141313"></a><a name="en-us_topic_0000002313328922_p1511719141313"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p711741414318"><a name="en-us_topic_0000002313328922_p711741414318"></a><a name="en-us_topic_0000002313328922_p711741414318"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p3117201420317"><a name="en-us_topic_0000002313328922_p3117201420317"></a><a name="en-us_topic_0000002313328922_p3117201420317"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p111731415319"><a name="en-us_topic_0000002313328922_p111731415319"></a><a name="en-us_topic_0000002313328922_p111731415319"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1117814153111"><a name="en-us_topic_0000002313328922_p1117814153111"></a><a name="en-us_topic_0000002313328922_p1117814153111"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p11178144316"><a name="en-us_topic_0000002313328922_p11178144316"></a><a name="en-us_topic_0000002313328922_p11178144316"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p12118141419319"><a name="en-us_topic_0000002313328922_p12118141419319"></a><a name="en-us_topic_0000002313328922_p12118141419319"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p5118201443110"><a name="en-us_topic_0000002313328922_p5118201443110"></a><a name="en-us_topic_0000002313328922_p5118201443110"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p5118014193110"><a name="en-us_topic_0000002313328922_p5118014193110"></a><a name="en-us_topic_0000002313328922_p5118014193110"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p41180146312"><a name="en-us_topic_0000002313328922_p41180146312"></a><a name="en-us_topic_0000002313328922_p41180146312"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p8118141411318"><a name="en-us_topic_0000002313328922_p8118141411318"></a><a name="en-us_topic_0000002313328922_p8118141411318"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p10118101413114"><a name="en-us_topic_0000002313328922_p10118101413114"></a><a name="en-us_topic_0000002313328922_p10118101413114"></a>`torch.int8`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p51181614113119"><a name="en-us_topic_0000002313328922_p51181614113119"></a><a name="en-us_topic_0000002313328922_p51181614113119"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p18118181493113"><a name="en-us_topic_0000002313328922_p18118181493113"></a><a name="en-us_topic_0000002313328922_p18118181493113"></a>`torch.bfloat16`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1111920146316"><a name="en-us_topic_0000002313328922_p1111920146316"></a><a name="en-us_topic_0000002313328922_p1111920146316"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.080908090809082%"><p id="en-us_topic_0000002313328922_p1611918142315"><a name="en-us_topic_0000002313328922_p1611918142315"></a><a name="en-us_topic_0000002313328922_p1611918142315"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p6119101414312"><a name="en-us_topic_0000002313328922_p6119101414312"></a><a name="en-us_topic_0000002313328922_p6119101414312"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="9.09090909090909%"><p id="en-us_topic_0000002313328922_p1611911414313"><a name="en-us_topic_0000002313328922_p1611911414313"></a><a name="en-us_topic_0000002313328922_p1611911414313"></a>`torch.float`</p>
    </td>
    ...
    <td class="cellrowborder" valign="top" width="8.04080408040804%"><p id="en-us_topic_0000002313328922_p1911971463117"><a name="en-us_topic_0000002313328922_p1911971463117"></a><a name="en-us_topic_0000002313328922_p1911971463117"></a>`torch.float`</p>
    </td>
