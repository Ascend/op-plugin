...
- Performs Sigmoid or Softmax computation on the input `x` during MoE computation, groups and sorts the computation results, and selects the top-K experts based on the group sorting results.
- Formulas:
...
- **`renorm`** (`int`): Optional. Renormalization flag. The default value is `0` (performs normalization before the Top-K computation). Currently, only the value `0` is supported.
