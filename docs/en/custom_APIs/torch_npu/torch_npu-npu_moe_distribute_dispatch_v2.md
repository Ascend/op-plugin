...
- **`x`** (`Tensor`): Required. Token data used for computation and dispatched to other ranks based on `expert_ids`. This parameter must be 2D with shape `(BS, H)`, indicating that there are `BS` tokens. The data type can be `torch.bfloat16` or `torch.float16`. The data layout can be ND. Non-contiguous tensors are supported.
...
- **`moe_expert_num`** (`int`): Required. Number of MoE experts. The condition `moe_expert_num % (ep_world_size - shared_expert_rank_num) == 0` must be satisfied.
    - Atlas A2 training products/Atlas A2 inference products: The value range is [1, 1024].
    - Atlas A3 training products/Atlas A3 inference products: The value range is [1, 1024]. When `comm_alg` is set to `"hierarchy"`, the value range is [1, 512].
...
- **`expert_scales`** (`Tensor`): Optional. Top-K expert weights for each token.
    - Atlas A2 training products/Atlas A2 inference products: This parameter must be 2D with shape `(BS, K)`. The data type can be `torch.float32`. The data layout is ND. Non-contiguous tensors are supported.
    - Atlas A3 training products/Atlas A3 inference products: When `comm_alg` is set to `"hierarchy"`, this parameter must be 2D with shape `(BS, K)`. The data type can be `torch.float32`. The data layout is ND. Non-contiguous tensors are supported. When `comm_alg` is set to `""`, `"fullmesh_v1"`, or `"fullmesh_v2"`, this parameter is not supported. Use the default value.
...
- **`tp_rank_id`** (`int`): Optional. Rank ID of the current rank within the TP communication domain. This parameter is required only when TP domain communication is involved.
    - Atlas A2 training products/Atlas A2 inference products: TP domain communication is not supported. Use the default value.
    - Atlas A3 training products/Atlas A3 inference products: When TP domain communication is involved, the value range is [0, 1]. The default value is `0`. The `tp_rank_id` of each rank in the same TP communication domain must be unique. If there is no TP domain communication, pass `0`.
...
- **`comm_alg`** (`string`): Optional. Communication-optimized memory-layout algorithm.
    ...
    - Atlas A3 training products/Atlas A3 inference products: The current version supports `""`, `"fullmesh_v1"`, `"fullmesh_v2"`, or `"hierarchy"`.
...
- **`copy_expert_num`** (`int`): Optional. Number of copy experts.
    - Atlas A2 training products/Atlas A2 inference products:
         - When `comm_alg` is set to `"fullmesh"`, the value range is [0, `MAX_INT32`), where the value of `MAX_INT32` is `2147483647`. Valid copy-expert ID values are in the range [`moe_expert_num + zero_expert_num`, `moe_expert_num + zero_expert_num + copy_expert_num`). A non-zero value enables the copy expert feature, which is currently experimental and must be used with caution.
         - When `comm_alg` is set to `"hierarchy"`, this parameter is currently not supported. Use the default value `None`.
...
- **`expert_token_nums`** (`Tensor`): Number of tokens received by each expert on the current rank. It must be 1D with shape `(local_expert_num,)`. The data type can be `torch.int64`. The data layout can be ND. Non-contiguous tensors are supported.
...
- **`tp_recv_counts`** (`Tensor`): Number of tokens received by each rank in the TP communication domain. This parameter corresponds to the `tp_send_counts` input of [torch_npu.npu_moe_distribute_combine_v2](torch_npu-npu_moe_distribute_combine_v2.md).
    - Atlas A2 training products/Atlas A2 inference products: The TP communication domain is not supported. This output is not available.
    - Atlas A3 training products/Atlas A3 inference products: The TP communication domain is supported. This parameter must be 1D with shape `(tp_world_size,)`. The data type can be `torch.int32`. The data layout is ND. Non-contiguous tensors are supported.

- **`expand_scales`** (`Tensor`): Output after the alltoallv operation is performed on `expert_scales` and `x`.
    - Atlas A2 training products/Atlas A2 inference products: This parameter must be 1D with shape `(A,)`. The data type can be `torch.float32`. The data layout must be ND. Non-contiguous tensors are supported.
    - Atlas A3 training products/Atlas A3 inference products: When `comm_alg` is set to `"hierarchy"`, this parameter must be 1D with shape `(A,)`. The data type can be `torch.float32`. The data layout must be ND. Non-contiguous tensors are supported. When `comm_alg` is set to `""`, `"fullmesh_v1"`, or `"fullmesh_v2"`, this output is not supported. The default value `None` is returned.
...
- HCCL communication domain buffer size:
    ...
    - Atlas A3 training products/Atlas A3 inference products:
        This scenario supports configuration through both the `HCCL_BUFFSIZE` environment variable and [`hccl_buffer_size`](https://gitcode.com/Ascend/ModelZoo-PyTorch/blob/master/PyTorch/docs/en/performance_tuning/performance_tuning_methods/communication_basics_overview.md#hccl_buffer_size).
...
- Version mapping constraints:
     ...
     In static graph mode, starting with TorchNPU 8.0.0, the TorchNPU framework performs strict validation between the Meta inference results and inferShape inference results of the output of the last node in the static graph. If the graph contains only one Dispatch_v2 operator and the CANN version is earlier than the TorchNPU version, a shape mismatch error may occur. You are advised to upgrade the CANN version. For details about version compatibility information, see section **Related Product Versions** in the [TorchNPU Release Notes](https://gitcode.com/Ascend/pytorch/blob/v2.7.1-26.1.0/docs/en/release_notes/release_notes.md).
