...
- **`weight`** (`Tensor`): Filter with shape `(in_channels, out_channels/groups, kH, kW)` or `(in_channels, out_channels/groups, kT, kH, kW)`.
- **`bias`** (`Tensor`): Optional. Bias tensor with shape `(out_channels)`.
- **`padding`** (`List[int]`): Pads both sides of each dimension of the input with zeros based on `(dilation * (kernel_size - 1) - padding)`.
