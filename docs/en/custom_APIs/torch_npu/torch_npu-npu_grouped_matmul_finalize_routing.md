...
The inputs to GroupedMatMul typically include an input tensor `x`, expert weights `w`, and grouping information `group_list`. The shape of `x` is `(M, K)`, where `M` represents the total number of tokens and `K` represents the input feature dimension. The expert weights `w` can be viewed as a tensor containing the weights of $E$ experts. `group_list` describes the number of tokens processed by each expert or the prefix sums of the group sizes.

During computation, the operator partitions the rows of `x` into multiple contiguous groups based on `group_list`. For each expert, it takes the corresponding token group and performs a matrix multiplication with that expert's weights to obtain the expert's output. The outputs of all experts are concatenated in group order to produce the final GroupedMatMul output.

Subsequently, MoeFinalizeRouting combines the GroupedMatMul output into the target token positions according to `row_index`. When `logit` is provided, the expert output is first multiplied by the corresponding logit weight for the token before being combined. When `shared_input` is provided, the shared expert output participates in the final accumulation based on `shared_input_weight` and `shared_input_offset`.

This fused operator is commonly used in scenarios such as the FFN layers of MoE models, high-performance inference, and dynamic batching. It reduces zero padding, lowers operator scheduling overhead, and increases NPU throughput.
