...
- Routed expert formulas:
    ...
    - `send_counts` indicates the distribution of the number of tokens sent by the current rank to each expert shard in the EP communication domain during AlltoAllv.
    - `recv_counts` indicates the distribution of the number of tokens received by the current rank from each expert shard in the EP communication domain during AlltoAllv. It is used to determine the size of the first dimension of output `y`.
...
- Shared expert formula:
    ...
    - `mm_weight` indicates the right matrix for shared expert MatMul. When `trans_mm_weight` is set to `true`, the shared expert MatMul operation uses the transposed `mm_weight`.
...
- **`gmm_x`** (`Tensor`): Required. Left matrix for GroupedMatMul. The data type can be `torch.float16` or `torch.bfloat16`. This parameter must be 2D with shape `(A, H1)`. The data layout can be ND.
...
- **`send_counts`** (`List[int]`): Required. Token count distribution sent by the current rank to other ranks within the EP communication domain. The list length is fixed at `e * ep_world_size`, with a maximum length of 256. Each element is of type `int` and ranges from 0 to 52428800, representing the number of tokens sent to a specific expert shard. The sum of all elements equals the total number of tokens sent by the current rank, denoted by $A$.
- **`recv_counts`** (`List[int]`): Required. Token count distribution received by the current rank from other ranks within the EP communication domain. The list length is fixed at `e * ep_world_size`, with a maximum length of 256. Each element is of type `int` and ranges from 0 to 52428800, representing the number of tokens received from a specific expert shard. The sum of all elements equals the total number of tokens received by the current rank, denoted by $BSK$, which is used to determine the size of the first dimension of the output `y`.

  A brief calculation example for `send_counts`/`recv_counts` is as follows:
  ...
  BSK = sum(recv_counts)    # Total number of tokens received by the current rank; also the first dimension of output y
  # y.shape = (BSK, N1)
  ```
...
- **`mm_x`** (`Tensor`): Optional. Left matrix for shared expert MatMul. This parameter must be provided when shared expert computation is fused. The data type can be `torch.float16` or `torch.bfloat16`. This parameter must be 2D with shape `(BS, H2)`.
