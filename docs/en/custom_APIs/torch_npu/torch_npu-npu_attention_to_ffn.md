...
- **`session_id`** (`Tensor`): Required. Current rank ID within the Attention domain. This parameter must be 1D with shape `(X,)`. The data type can be `torch.int32`. The data layout is ND. Non-contiguous tensors are supported.
- **`micro_batch_id`** (`Tensor`): Required. ID of the current micro-batch group. This parameter must be 1D with shape `(X,)`. The data type can be `torch.int32`. The data layout is ND. Non-contiguous tensors are supported.
- **`layer_id`** (`Tensor`): Required. Model layer ID. This parameter must be 1D with shape `(X,)`. The data type can be `torch.int32`. The data layout is ND. Non-contiguous tensors are supported.
- **`expert_ids`** (`Tensor`): Required. Top-K expert indices for each token in each micro-batch group, which determines the destination experts for each token. This parameter must be 3D with shape `(X, BS, K)`. The data type can be `torch.int32`. The data layout is ND. Non-contiguous tensors are supported. The value range of elements inside the tensor is [0, `moe_expert_num`), and the $K$ values within the same row (same token) must be unique.
...
- Single-operator call
    ...
    moe_ffn_rank_num = 2    # Number of MOE FFN ranks
