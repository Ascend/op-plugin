...
- Formulas:
    ...
    - Grouping (`cumsum` or `count` modes are supported):
        ...
        Perform grouped computation on the result of applying SwiGLU to the input `x`. `group_index` is used to define group boundaries or the number of tokens in each group, depending on the `group_list_type` mode. A different quantization `smooth_scales` is used for each group.
