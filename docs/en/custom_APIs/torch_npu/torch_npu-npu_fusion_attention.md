...
> [!NOTICE]  
> This API has been updated in this version. For details, see [API Changes](https://gitcode.com/Ascend/pytorch/blob/v2.7.1-26.0.0/docs/en/release_notes/release_notes.md#api-changes).
...
- **`dropout_mask`** (`Tensor`): Optional. External dropout mask used to control dropout behavior. When this parameter is provided, the external mask is used (instead of the one internally generated) to achieve reproducible dropout effects. The data type can be `uint8`. The data layout can be ND. If this parameter is omitted, the operator automatically generates the dropout mask internally based on `seed` and `offset`. If this parameter is provided, the mask must be generated using the internal API [`_npu_dropout_gen_mask`](https://gitcode.com/Ascend/op-plugin/blob/26.0.0/op_plugin/ops/opapi/DropoutGenMaskKernelNpuOpApi.cpp).
