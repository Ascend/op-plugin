...
- Performs computation during Multi-Head Latent Attention (MLA) preprocessing in inference scenarios. This operator implements four parallel computation paths:
    1. Standard query path: Input $x$ → Downsampling through $W^{DQ}$ → `RmsNorm` → Upsampling through $W^{UQ}$ → Upsampling through $W^{UK}$ → $q^N$
    2. Position embedding query path: Input $x$ → Downsampling through $W^{DQ}$ → `RmsNorm` → $W^{QR}$ → Rotary position embedding (`ROPE`) → $q^R$
    3. Standard key path: Input $x$ → Downsampling through $W^{DKV}$ → `RmsNorm` → Committed to a cache layer → $k^C$
    4. Position embedding key path: Input $x$ → $W^{KR}$ → Rotary position embedding (`ROPE`) → Committed to a cache layer → $k^R$
...
- The main changes compared with `torch_npu.npu_mla_prolog_v2` are as follows:
    ...
    - `pertoken-pergroup` quantization mode is added for `kv_cache`.
...
- Formulas:
    ...
    - Path 2: Position embedding query computation

        This path applies `ROPE` rotary position embedding to the query.
    ...
    - Path 4: Position embedding key computation

        This path applies `ROPE` rotary position embedding to the key. It commits the computation results to a cache layer.
...
- **`tile_size`** (`int`): Optional. Size of each tile in `pertoken-pergroup` quantization. This parameter is valid only when `kv_cache_quant_mode` is set to `3`. The default value is `128`. It can be omitted when `kv_cache_quant_mode` is set to `1` or `2`.
...
- **`token_x_dtype`** (`int`): Optional. Input `dtype` of `token_x`. In full HiF8 quantization scenarios, this parameter is `torch_npu.hifloat8`; in other scenarios, it is `None`.

- **`weight_dq_dtype`** (`int`): Optional. Input `dtype` of `weight_dq`. In full HiF8 quantization scenarios, this parameter is `torch_npu.hifloat8`; in other scenarios, it is `None`.

- **`weight_uq_qr_dtype`** (`int`): Optional. Input `dtype` of `weight_uq_qr`. In full HiF8 quantization scenarios, this parameter is `torch_npu.hifloat8`; in other scenarios, it is `None`.

- **`weight_dkv_kr_dtype`** (`int`): Optional. Input `dtype` of `weight_dkv_kr`. In full HiF8 quantization scenarios, this parameter is `torch_npu.hifloat8`; in other scenarios, it is `None`.

- **`kv_cache_dtype`** (`int`): Optional. Input `dtype` of `kv_cache`. In scenarios involving `pertensor` HiF8 quantization or `pertoken-pergroup` HiF8 quantization of `kv_cache`, this parameter is `torch_npu.hifloat8`; in other scenarios, it is `None`.
...
- The following table describes the shape layout fields.
    ...
    | He           | Head-Size (hidden layer size)       | Valid values: `1024`, `2048`, `3072`, `4096`, `5120`, `6144`, `7168`, `7680`, or `8192`.         |
    ...
    | Dtile        | D-axis dimension of `kv_cache`      | The value is fixed at `656` in `pertoken-pergroup` mode and `512` in non-`pertoken-pergroup` modes. |
    | D            | QK dimension without position encoding           | Valid values: `128` or `192`.                                                            |
    | Dr           | QK position embedding dimension               | The value is fixed at `64`.                                                             |
    | Nkv          | Number of KV heads                 | The value is fixed at `1`.                                                              |
    | BlockNum     | Number of `pertoken-pergroup` blocks in the PagedAttention scenario   | The value is rounded up to the nearest integer after the result of `B*Skv/BlockSize` is calculated. (`Skv` indicates the sequence length of KV, which can be `0`.)|
...
- Supported scenarios:
  ...
  <table style="table-layout: auto;" border="1">
    ...
    <tr>
      <td><code>kv_cache</code> quantized in <code>pertoken-pergroup</code> mode</td>
      <td>
          - weight_quant_mode=3, kv_cache_quant_mode=3, query_quant_mode=0<br>
          - Inputs: <code>weight_uq_qr</code> must be provided as <code>pertoken</code> quantized data, <code>kv_cache</code> must be provided as <code>pertoken-pergroup</code> quantized data, and all other inputs must be non-quantized data.<br>
          The <code>dequant_scale_w_uq_qr</code> and <code>quant_scale_ckv</code> fields are required, and the <code>smooth_scales_cq</code> field is optional.<br>
          - Outputs: <code>kv_cache_out</code> is <code>pertoken-pergroup</code> quantized data, and all other outputs are non-quantized data.
      </td>
    </tr>
    <tr>
      ...
      <td>
          ...
          The <code>dequant_scale_x</code>, <code>dequant_scale_w_dq</code>, <code>dequant_scale_w_uq_qr</code>, and <code>dequant_scale_w_dkv_kr</code> fields are required, and the <code>smooth_scales_cq</code> field is optional.<br>
          - Outputs: All outputs are non-quantized data.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
          ...
          - Inputs: <code>token_x</code> must be provided as pertoken quantized data, <code>weight_dq</code>, <code>weight_uq_qr</code>, and <code>weight_dkv_kr</code> must be provided as <code>perchannel</code> quantized data, and all other inputs must be non-quantized data.<br>
          The <code>dequant_scale_x</code>, <code>dequant_scale_w_dq</code>, <code>dequant_scale_w_uq_qr</code>, and <code>dequant_scale_w_dkv_kr</code> are required, and the <code>smooth_scales_cq</code> field is optional.<br>
          - Outputs: <code>query_out</code> returns per-token-per-head quantized data, <code>kv_cache</code> returns pertoken-pergroup quantized data, and all other outputs return non-quantized data
      </td>
    </tr>
    <tr>
      <td rowspan="3"><em>Full MXFP8 quantization</em></td>
...
- The following table describes the parameter dtype constraints in different quantization scenarios.
  ...
  <table style="table-layout: auto;" border="1">
    ...
    <tr>
      ...
      <th><code>kv_cache</code> pertoken-pergroup quantized<br>dtype</th>
      <th><code>kv_cache</code> non-quantized<br>dtype</th>
      <th><code>kv_cache</code> quantized<br>dtype</th>
      <th><code>kv_cache</code> pertoken-pergroup quantized<br>dtype</th>
      <th><code>kv_cache</code> non-quantized<br>dtype</th>
      <th><code>kv_cache</code> pertensor quantized<br>dtype</th>
      <th><code>kv_cache</code> pertoken-pergroup quantized<br>dtype</th>
      <th><code>kv_cache</code> non-quantized<br>dtype</th>
      <th><code>kv_cache</code> pertensor quantized<br>dtype</th>
      <th><code>kv_cache</code> pertoken-pergroup quantized<br>dtype</th>
