...
- Implements full FlashAttention (FA).
...
- **`query`** (`Tensor`): Required. Input $Q$ in the formulas. The data type must satisfy type deduction rules to remain identical to the data types of `key` and `value`. Non-contiguous tensors are not supported. The data layout can be ND.
    ...
    - Atlas A3 training products/Atlas A3 inference products: The data type can be `torch.float16`, `torch.bfloat16`, or `torch.int8`.
...
- **`deq_scale1`** (`Tensor`): Optional. Dequantization factor after BMM1. `pertensor` mode is supported. The data type can be `torch.uint64` or `torch.float32`. The data layout can be ND. This parameter can be set to `None` if this feature is not used. Atlas inference accelerator cards: This parameter is not supported currently.
- **`quant_scale1`** (`Tensor`): Optional. Quantization factor before BMM2. `pertensor` mode is supported. The data type can be `torch.float32`. The data layout can be ND. This parameter can be set to `None` if this feature is not used. Atlas inference accelerator cards: This parameter is not supported currently.
- **`deq_scale2`** (`Tensor`): Optional. Dequantization factor after BMM2. `pertensor` mode is supported. The data type can be `torch.uint64` or `torch.float32`. The data layout can be ND. This parameter can be set to `None` if this feature is not used. Atlas inference accelerator cards: This parameter is not supported currently.
...
- **`num_heads`** (`int`): Optional. Head count of `query`. The data type can be `torch.int64`.
...
- **`pre_tokens`** (`int`): Optional. Number of preceding tokens to associate in attention computation for sparse computation. The data type can be `torch.int64`. The default value is `2147483647`. Only the default value `2147483647` is supported for Atlas inference accelerator cards.
- **`next_tokens`** (`int`): Optional. Number of subsequent tokens to associate in attention computation for sparse computation. The data type can be `torch.int64`. The default value is `0`. Only `0` and `2147483647` are supported for Atlas inference accelerator cards.
...
- **`num_key_value_heads`**: Optional. Number of heads for `key` and `value`, used to support Grouped-Query Attention (GQA) scenarios. GQA is a compromise between Multi-Head Attention (MHA) and Multi-Query Attention (MQA): multiple query heads share the same set of key/value heads, thereby significantly reducing KV Cache memory usage and memory access overhead while maintaining model performance close to that of MHA. The data type can be `torch.int64`. The default value is `0`, indicating that the head counts of `key`, `value`, and `query` are identical. Restrictions: `num_heads` must be divisible by `num_key_value_heads`. The ratio of `num_heads` to `num_key_value_heads` must be less than or equal to `64`. In `"BSND"`, `"BNSD"`, or `"BNSD_BSND"` layouts, this value must be identical to the shape value of the n-axis in `key`/`value`. If this condition is not satisfied, the system raises an error. Only the default value `0` is supported for Atlas inference accelerator cards.
- **`actual_seq_lengths_kv`** (`List[int]`): Optional. Valid sequence lengths of `key`/`value` across different batches. The data type can be `torch.int64`. Restrictions: Valid sequence lengths in each batch within this parameter must be less than or equal to the sequence length of the corresponding batch in `key`/`value`. When the input length of `seqlenKV` is `1`, each batch uses the same `seqlenKV`. When the input length is greater than or equal to the batch count, the first batch number of elements are selected. Other lengths are not supported.
    - Atlas inference accelerator cards: This parameter is not supported currently.
...
- **`sparse_mode`** (`int`): Optional. Sparsification mode. The data type can be `torch.int64`. The default value is `0`. For details about overall constraints, see [Constraints](#section12345537164214). Only the default value `0` is supported for Atlas inference accelerator cards.
...
- Overall constraints on the number of `torch.int8` quantization-related parameters and the input or output data formats:
    - When the input is `torch.int8` and the output is `torch.int8`, the input parameters `deq_scale1`, `quant_scale1`, `deq_scale2`, and `quant_scale2` must all be provided. `quant_offset2` is optional. If omitted, the default value `0` is used.
    - When the input is `torch.int8` and the output is `torch.float16`, the input parameters `deq_scale1`, `quant_scale1`, and `deq_scale2` must all be provided. If `quant_offset2` or `quant_scale2` is provided, an error is raised and returned.
    - When the input is `torch.float16` or `torch.bfloat16` and the output is `torch.int8`, the input parameter `quant_scale2` must be provided. `quant_offset2` is optional. If omitted, the default value `0` is used. If `deq_scale1`, `quant_scale1`, or `deq_scale2` is provided (not set to `None`), an error is raised and returned.
    ...
    - In `perchannel` mode, the input parameters `quant_scale2` and `quant_offset2` do not support left padding, Ring Attention, or scenarios where the $D$ dimension is not 32-byte aligned. Ring Attention is a distributed attention computation approach for ultra-long sequences that partitions the KV sequence across multiple devices and completes attention computation by passing KV blocks between devices in a ring.
    - When the output is `torch.int8`, the scenario where the sparse mode is `"band"` and `pre_tokens` or `next_tokens` is negative is not supported.
...
- When the output is `torch.int8`, the input parameter `quant_offset2` is provided as a non-null pointer and a non-empty `Tensor`, and `sparse_mode`, `pre_tokens`, and `next_tokens` meet the following conditions, some matrix rows do not participate in computation. This causes computation errors, and execution will be blocked:
