...
Marks the beginning of the task to be updated in `NPUGraph` scenarios.
...
- **`handle`**: Required. Task group handle generated by the [torch_npu.npu.graph_task_group_end](torch_npu-npu-graph_task_group_end.md) API.
...
- In the graph update phase, use this API together with [torch_npu.npu.graph_task_update_end](torch_npu-npu-graph_task_update_end.md) to mark the beginning and end of the task to be updated.
- The stream in the graph update phase must be different from that in the graph capture phase.
