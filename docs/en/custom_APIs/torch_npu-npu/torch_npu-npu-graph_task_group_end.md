...
Marks the end of a task group in `NPUGraph` scenarios.
...
## Return Values
 ...
 A handle identifying the task group. This handle is used as the input to the [torch_npu.npu.graph_task_update_begin](torch_npu-npu-graph_task_update_begin.md) API.

## Constraints

- In the graph capture phase, use this API together with [torch_npu.npu.graph_task_group_begin](torch_npu-npu-graph_task_group_begin.md) to generate a task group handle.
- The input `stream` must be the same as that of [torch_npu.npu.graph_task_group_begin](torch_npu-npu-graph_task_group_begin.md).
