...
<!-- md-trans-meta sourceCommit=c6d785c77c92c77d7fc800bb1620ff277b5750a9 translatedAt=2026-08-11T01:29:39.261Z pushedAt=2026-08-11T12:00:13.550Z -->
...
1. Implement the C++ operator code, adaptation layer, custom operator schema registration, and implementation binding in the `add_custom.cpp` file under the `csrc` directory. The code sample is as follows:
...
2. Add the operator call logic and load the `.so` file in the `__init__.py` and `ops/__init__.py` files under the `cpp_extension_base` directory.
