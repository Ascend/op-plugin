...
// 为NPU设备注册反向实现
std::tuple<at::Tensor, at::Tensor> add_custom_backward_impl_npu(
...
// 为Meta设备注册前向实现
at::Tensor add_custom_impl_meta(
    const at::Tensor& self,
    const at::Tensor& other) {
...
// 为Meta设备注册反向实现
std::tuple<at::Tensor, at::Tensor> add_custom_backward_impl_meta(
    const at::Tensor& self) {
...
class AddCustomFunction : public torch::autograd::Function<AddCustomFunction> {
 public:
  static at::Tensor forward(
    ...
    static auto op =
        torch::Dispatcher::singleton()
            .findSchemaOrThrow("cpp_extension_full::add_custom", "")
  ...
  static variable_list backward(
    ...
    static auto op =
        torch::Dispatcher::singleton()
            .findSchemaOrThrow("cpp_extension_full::add_custom_backward", "")
...
// 使用的时候调用apply()方法
at::Tensor add_custom_autograd(
    const at::Tensor& self,
    const at::Tensor& other) {
  return AddCustomFunction::apply(self, other);
}

// 为Meta设备创建专门的Function类
class AddCustomMetaFunction
    : public torch::autograd::Function<AddCustomMetaFunction> {
 public:
  static at::Tensor forward(
      AutogradContext* ctx,
      at::Tensor self,
    ...
    static auto op =
        torch::Dispatcher::singleton()
            .findSchemaOrThrow("cpp_extension_full::add_custom", "")
            .typed<decltype(add_custom_impl_meta)>(); // 注意这里用meta实现
  ...
  static variable_list backward(
    ...
    static auto op =
        torch::Dispatcher::singleton()
            .findSchemaOrThrow("cpp_extension_full::add_custom_backward", "")
            .typed<
                decltype(add_custom_backward_impl_meta)>(); // 注意这里用meta实现

    auto result = op.call(grad_output);

    // 返回两个梯度，分别对应self和other
    return {std::get<0>(result), std::get<1>(result)};
  }
};

// Meta设备的autograd wrapper函数
at::Tensor add_custom_autograd_meta(
    const at::Tensor& self,
    const at::Tensor& other) {
  return AddCustomMetaFunction::apply(self, other);
}
