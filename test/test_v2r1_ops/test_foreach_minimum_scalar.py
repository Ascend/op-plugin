...
class TestForeachMinimumScalar(TestCase):
    ...
    def test_foreach_maximum_scalar_out_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalar = 5.0
            cpu_output = torch._foreach_maximum(cpu_tensors[0], scalar)
    ...
    def test_foreach_maximum_scalar_out_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    def test_foreach_maximum_scalar_inplace_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalar = 5.0
            torch._foreach_maximum_(cpu_tensors[0], scalar)
