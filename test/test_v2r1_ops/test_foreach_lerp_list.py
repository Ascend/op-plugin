...
class TestForeachLerpList(TestCase):
    ...
    def test_foreach_lerp_list_out_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_lerp_list_inplace_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "bfloat16")
            torch._foreach_lerp_(cpu_tensors[0], cpu_tensors[1], cpu_tensors[2])
