...
class TestForeachAddcmulList(TestCase):
    ...
    def create_input_scalars(self, tensor_nums, dtype):
        if dtype == "int32":
            return torch.randint(low=0, high=100, size=(tensor_nums,), dtype=self.torch_dtypes.get(dtype))
        return torch.randn((tensor_nums), dtype=self.torch_dtypes.get(dtype))

    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_out_float32_shpae_tensor_num(self):
        ...
        for tensor_num in tensor_num_list :
            ...
            cpu_output = torch._foreach_addcmul(cpu_tensors[0], cpu_tensors[1], cpu_tensors[2], scalars)
            npu_output = torch._foreach_addcmul(npu_tensors[0], npu_tensors[1], npu_tensors[2], scalars)
            
            self.assertRtolEqual(cpu_output, npu_output)
    
    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_out_float16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            ...
            cpu_tensors_4 = [cpu_tensor.numpy() for cpu_tensor in scalars]
            cpu_output = [torch.from_numpy(cpu_tensors_1[i] + cpu_tensors_2[i] * cpu_tensors_3[i] * cpu_tensors_4[i]) for i in range(len(scalars))]
            npu_output = torch._foreach_addcmul(npu_tensors[0], npu_tensors[1], npu_tensors[2], scalars)

            self.assertRtolEqual(cpu_output, npu_output)

    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_out_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_inplace_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalars = self.create_input_scalars(tensor_num, "float32")
            torch._foreach_addcmul_(cpu_tensors[0], cpu_tensors[1], cpu_tensors[2], scalars)
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_inplace_float16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float16")
            scalars = self.create_input_scalars(tensor_num, "float16")
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_inplace_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "bfloat16")
            scalars = self.create_input_scalars(tensor_num, "bfloat16")
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_list_inplace_int32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
