...
class TestForeachClampMaxScalar(TestCase):
    ...
    def test_foreach_clamp_max_scalar_out_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalar = 5.0
            cpu_output = torch._foreach_clamp_max(cpu_tensors[0], scalar)
            npu_output = torch._foreach_clamp_max(npu_tensors[0], scalar)

            self.assertRtolEqual(cpu_output, npu_output)

    def test_foreach_clamp_max_scalar_out_float16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    def test_foreach_clamp_max_scalar_out_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    def test_foreach_clamp_max_scalar_out_int32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    def test_foreach_clamp_max_scalar_inplace_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalar = 5.0
            torch._foreach_clamp_max_(cpu_tensors[0], scalar)
            torch._foreach_clamp_max_(npu_tensors[0], scalar)

            self.assertRtolEqual(cpu_tensors[0], npu_tensors[0])

    def test_foreach_clamp_max_scalar_inplace_float16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float16")
            scalar = 5.0
    ...
    def test_foreach_clamp_max_scalar_inplace_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "bfloat16")
            scalar = 5.0
    ...
    def test_foreach_clamp_max_scalar_inplace_int32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "int32")
            scalar = 5
