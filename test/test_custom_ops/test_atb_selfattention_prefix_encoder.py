...
class TestFlashAttentionPrefixEncoder(TestCase):
    ...
    def set_data_params(self, dynamic_batch=False, batch_state=None, window_size=0, cache_type=0,
                        ...
                        block_size=None, model_case=False):
        ...
        if num_blocks is None:
            ...
            for i in range(batch):
                num_blocks_cur_seq = (
                    kv_seqLen[i] + block_size - 1) // block_size
                # padding block table with 0
                block_table = [random.randint(0, num_blocks - 1) if j < num_blocks_cur_seq else 0 for j in range(max_num_blocks_per_seq)]
    ...
    def gen_out_tensor(self, online=False):
        ...
        for idx in range(batch):
            ...
            o_res = self.group_mm_torch(heads, kv_head, p, v_slice)
            if self.fav3:
                o_res = o_res.to(torch.float)
                v_scale = self.v_scale
                v_scale = v_scale.view(heads, 1, 1)
                o_res = o_res * v_scale
                o_res = o_res / 127
                o_res = o_res / score_sum.reshape((heads, q_s, 1))
            else:
                o_true = o_true.view(heads, q_s, embedv)
                o_true = torch.permute(o_true, (1, 0, 2)).contiguous()
            o_res = o_res.view(heads, q_s, embedv)
            o_res = torch.permute(o_res, (1, 0, 2)).contiguous()
            if out is None:
                out = o_res
                if not self.fav3:
                    out_true = o_true
            else:
                out = torch.cat((out, o_res), 0)
                if not self.fav3:
    ...
    def test_flash_attention_case_fa_encoder_withcache_bf16_maskfree(self):
        ...
        self.gen_out_tensor()
        query = self.q.npu()
        key = self.k_cache.npu()
        value = self.v_cache.npu()
        block_tables = self.block_tables.npu()
        q_seqlen = np.array(q_seqlens)
        q_seqlen = torch.from_numpy(q_seqlen).to(torch.int32)
        kv_seqLen = np.array(kv_seqLen)
        kv_seqLen = torch.from_numpy(kv_seqLen).to(torch.int32)
        output = torch_npu.atb.npu_self_attention_prefix_encoder(
            query, key, value, block_tables, q_seqlen, kv_seqLen, heads, tor, kv_head)
        self.assertRtolEqual(output, self.golden_out)
    ...
    @unittest.skip("Skipping due to outdated CANN version; please update CANN to the latest version and remove this skip")
    @SupportedDevices(['Ascend910B'])
    def test_flash_attention_case_fa_encoder_model_case(self):
        batch = 92
        ...
        self.set_data_params(dynamic_batch=dynamic_batch,
                             ...
                             op_type=OP_PARAM["type"], mask_type=MASK_TYPE_ALIBI_NO_BATCH_WITH_PREFIX,
                             no_cache=True, tor=tor, q_seqlens=q_seqlens,
                             num_blocks=num_blocks, block_size=block_size, is_triu_mask=True, is_mask=True, model_case=True)
        self.gen_out_tensor()
        query = self.q.npu()
        key = self.k_cache.npu()
        value = self.v_cache.npu()
        ...
        self.assertTrue(self.compare_output_data(output.cpu(), self.golden_out, ratios))
