...
class TestAllGatherQuantMm(TestCase):
    ...
    def _test_npu_all_gather_quant_mm(cls, rank, input_list):
        x1_list, x2_list, ydtype, world_size, init_pg, c2p = input_list
        x1 = x1_list[rank]
        x2 = x2_list[rank]
        ...
        try:
            ...
            has_expected_error = any(kw in error_msg for kw in expected_error_keywords)
            if has_expected_error:
                print(f"[Rank {rank}] Expected RuntimeError caught: {e}")
                c2p.put((rank, None, None, "error_expected"))
            else:
                print(f"[Rank {rank}] Unexpected RuntimeError: {e}")
                raise

        except Exception as e:
            print(f"[Rank {rank}] Unexpected general exception: {e}"  )
            raise

        pg.barrier()

    def _test_multiprocess(self, f, init_pg, input_list):
        expt_out_list, expt_gather, x1, x2, ydtype, world_size = input_list
        ...
        for i in range(world_size):
            p = ctx.Process(
                target=f,
                args=(i, [x1, x2, ydtype, world_size, init_pg, c2p]))
            p.start()
            ps.append(p)
    ...
    def test_npu_all_gather_quant_mm(self):
        ...
        for _ in range(world_size):
            ...
            # output, _ = create_common_tensor(output_dtype, -1, 1)
            x1_list.append(x1)
            x2_list.append(x2)
        ...
        self._test_multiprocess(TestAllGatherQuantMm._test_npu_all_gather_quant_mm,
                                TestAllGatherQuantMm._init_dist_hccl, [expt_out_list, expt_gather, x1_list, x2_list, ydtype, world_size])
