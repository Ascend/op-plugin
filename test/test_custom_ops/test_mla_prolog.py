...
class TestPromptFlashAttetion(TestCase):
    def baseline(self, token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq,
            rmsnorm_gamma_ckv, rope_sin, rope_cos, cache_index, kv_cache, kr_cache, dequant_scale_w_uq_qr,
            smooth_scales_cq, rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode, mla_param):
        ...
        def dynamic_quantization(input_tensor, smooth_scale):
            ...
            if len(smooth_scale.shape) != 2 or smooth_scale.shape[1] != H:
                print(f"[ERROR]dynamic_quantization got wrong input smooth_scale:{tuple(smooth_scale.shape)}, expected:({H})")
                return None, None
            input_tensor = input_tensor.reshape(T, H).to(torch.float32)
            smooth_scale = smooth_scale.to(torch.float32)
            for bs_index in range(T):
                abs_bs_tensor = torch.abs(input_tensor[bs_index, :] * smooth_scale[0, :])
                scale_bs = abs_bs_tensor.max() / 127
                scale[bs_index] = scale_bs
                y[bs_index:] = torch.round(input_tensor[bs_index:] * smooth_scale[0, :] / scale_bs)
            return y, scale
        ...
        # matmul1 : token_x(B*S1,He) * w_dq (He,Hcq) -> matmul_res1(B*S1,Hcq)
        w_dq = weight_dq
        ...
        smo_scale_cq = smooth_scales_cq
        norm1_res, deq_scale_qcqr = dynamic_quantization(norm1_res, smo_scale_cq)
        matmul2_res = torch.matmul(norm1_res, w_uq_qr).to(torch.float32)
        deq_scale_uqqr = dequant_scale_w_uq_qr
        ...
        query_mla = query_mla.transpose(0, 1).reshape(B, S1, N1, Hckv)
        ...
        query_rope_mla = query_rope_mla.reshape(B, S1, N1, Dr)
    ...
    def mla_prolog_npu(self, token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq,
            rmsnorm_gamma_ckv, rope_sin, rope_cos, cache_index, kv_cache, kr_cache, dequant_scale_w_uq_qr,
            smooth_scales_cq, rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode):

        return torch_npu.npu_mla_prolog(
            token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq,
            rmsnorm_gamma_ckv, rope_sin, rope_cos, cache_index, kv_cache, kr_cache, dequant_scale_w_uq_qr=dequant_scale_w_uq_qr,
            smooth_scales_cq=smooth_scales_cq, rmsnorm_epsilon_cq=rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv=rmsnorm_epsilon_ckv, cache_mode=cache_mode)
    ...
    def test_op_exec(self):
        ...
        S = 2
        Nkv = 1
        token_x = torch.rand(B, S, He, dtype=torch.bfloat16)
        w_dq = torch.rand(He, Hcq, dtype=torch.bfloat16)
        w_uq_qr = torch.randint(0, 128, (Hcq, N * (D + Dr))).to(torch.int8)
        w_uk = torch.rand(N, D, Hckv, dtype=torch.bfloat16)
        w_dkv_kr = torch.rand(He, Hckv + Dr, dtype=torch.bfloat16)
        rmsnorm_gamma_cq = torch.rand(Hcq, dtype=torch.bfloat16)
        rmsnorm_gamma_ckv = torch.rand(Hckv, dtype=torch.bfloat16)
        rope_sin = torch.rand(B, S, Dr, dtype=torch.bfloat16)
        rope_cos = torch.rand(B, S, Dr, dtype=torch.bfloat16)
        cache_index = torch.rand(B, S).to(torch.int64)
        kv_cache = torch.rand(B, Nkv, Skv, Hckv, dtype=torch.bfloat16)
        kr_cache = torch.rand(B, Nkv, Skv, Dr, dtype=torch.bfloat16)
        dequant_scale_w_uq_qr = torch.rand(1, N * (D + Dr), dtype=torch.float)
        smooth_scales_cq = torch.rand(1, Hcq, dtype=torch.float)
        ...
        query_mla, query_rope_mla, kv_cache_out_mla, kr_cache_out_mla = self.mla_prolog_npu(token_x.npu(), w_dq.npu(), w_uq_qr.npu(),
            w_uk.npu(), w_dkv_kr.npu(), rmsnorm_gamma_cq.npu(), rmsnorm_gamma_ckv.npu(), rope_sin.npu(), rope_cos.npu(),
            cache_index.npu(), kv_cache.npu(), kr_cache.npu(), dequant_scale_w_uq_qr.npu(), smooth_scales_cq.npu(),
            rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode)
        print("mla_prolog output", query_mla, query_mla.shape)

        query, query_rope, kv_cache_out, kr_cache_out = self.baseline(token_x, w_dq, w_uq_qr, w_uk, w_dkv_kr, rmsnorm_gamma_cq,
            rmsnorm_gamma_ckv, rope_sin, rope_cos, cache_index, kv_cache, kr_cache, dequant_scale_w_uq_qr,
