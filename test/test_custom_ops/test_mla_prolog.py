...
class TestPromptFlashAttetion(TestCase):
    def baseline(self, token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq,
        ...
        w_dq = w_dq.unsqueeze(0).expand(B, -1, -1).clone()
        ...
        w_uq_qr = w_uq_qr.unsqueeze(0).expand(B, -1, -1).clone()
        matmul2_res = torch.matmul(norm1_res, w_uq_qr).to(torch.float32)
        matmul2_res = matmul2_res.reshape(B, S1, N1, D + Dr)
        ...
        splitd1_res1 = splitd1_res1.reshape(B * S1, N1, D).transpose(0, 1)
        query_mla = torch.zeros((N1, B * S1, Hckv))
        ...
        expanded_cos = cos.unsqueeze(2).repeat(1, 1, N1, 1)
        expanded_sin = sin.unsqueeze(2).repeat(1, 1, N1, 1)
        q = splitd1_res2.reshape(B, S1, N1, int(Dr / 2), 2).transpose(4, 3).reshape(B, S1, N1, Dr)
        ...
        token_x = token_x.reshape(B * S1, He)
        matmul4_res = torch.matmul(token_x, w_kv_kr).to(torch.float32)

        matmul4_res = matmul4_res.to(torch.float32) # .to(torch.bfloat16).to(torch.float32)
        matmul4_res = matmul4_res.reshape(B, S1, Hckv + Dr)
        ...
        gamma2 = rmsnorm_gamma_ckv.to(torch.float32)
        ...
        index_table_list = index_table.reshape(B * S1)
        ...
        k = splitd2_res2.reshape(B, S1, 1, int(Dr / 2), 2).transpose(4, 3).reshape(B, S1, Dr)
