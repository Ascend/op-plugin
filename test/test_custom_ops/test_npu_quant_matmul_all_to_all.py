...
class TestQuantMatmulAlltoAll(TestCase):
    ...
    @classmethod
    def _test_npu_quant_matmul_all_to_all(cls, rank, input_list):
        # Unpack inputs
        x1, x2, x1_scale, x2_scale, bias, world_size, init_pg, c2p = input_list
        ...
        # Call NPU operator
        out = torch_npu.npu_quant_matmul_all_to_all(
            x1, x2, hcom_name, world_size, bias, x1_scale, x2_scale, common_scale, x1_offset, x2_offset,
            x1_quant_mode, x2_quant_mode, common_quant_mode, group_size, all2all_axes, comm_quant_dtype,
            x1_dtype, x2_dtype, x1_scale_dtype, x2_scale_dtype, output_scale_dtype, comm_scale_dtype, y_dtype)
    ...
    def _test_multiprocess(self, f, init_pg, input_list): # f function, init_pg function, input_list list[]
        expt_out_list, x1_list, x2_list, x1_scale_list, x2_scale_list, bias_list, world_size = input_list
        ...
        for i in range(world_size):
            p = ctx.Process(
                target=f,
                args=(i, [x1_list[i], x2_list[i], x1_scale_list[i], x2_scale_list[i], bias_list[i], world_size, init_pg, c2p])
            )
            p.start()
            ps.append(p)
    ...
    def _construct_excepted_result(self, x1_list, x2_list, x1_scale_list, x2_scale_list, bias_list, world_size):
        # 存储每个rank的golden输出
        ...
        for i in range(world_size):
            ...
            # 7、还原通信结果为MatmulAlltoAll算子的实际输出shape，[BS*rankSize,H2/rankSize]
    ...
    def test_npu_quant_matmul_all_to_all(self):
        # 设备卡数
        world_size = 2
        # 初始化dtype，构造如下：输入x1和x2为torch.float8_e4m3fn，scale、bias和output为float32
        xDtype = torch.float8_e4m3fn  # Simulate supported dtype
        ...
        x1_shape = [xDtype, data_format, [m, k]]
        x2_shape = [xDtype, data_format, [k, n]]
        x1_scale_shape = [oDtype, data_format, [m]]
        ...
        expt_out_list = self._construct_excepted_result(x1_list, x2_list, x1_scale_list, x2_scale_list, bias_list, world_size)
        # 启动多线程
        self._test_multiprocess(
            TestQuantMatmulAlltoAll._test_npu_quant_matmul_all_to_all,
            TestQuantMatmulAlltoAll._init_dist_hccl,
            [expt_out_list, x1_list, x2_list, x1_scale_list, x2_scale_list, bias_list, world_size]
        )
