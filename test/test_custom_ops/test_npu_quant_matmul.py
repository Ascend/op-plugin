...
class TestQuantMatmul(TestCase):
    ...
    def supported_op_exec(self, x1, x2, uint64_deq_scale):
    ...
    def test_npu_quant_matmul(self, device="npu"):
        ...
        x2 = torch.randn(320, 2560, dtype=torch.int8).npu()
        deq_scale_shape = (1,)
        x1_clone = x1.clone()
        x2_clone = x2.clone()
        uint64_deq_scale = self.deq_scale_generate(deq_scale_shape, 'float16')

        supported_output = self.supported_op_exec(x1, x2, uint64_deq_scale)
        custom_output = self.custom_op_exec(x1_clone, x2_clone, uint64_deq_scale)
