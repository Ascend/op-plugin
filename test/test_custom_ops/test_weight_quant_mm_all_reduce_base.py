...
class TestMmAllReduceBase(TestCase):
    ...
    def _construct_excepted_result(self, x1_list, x2_list, scale, offset, world_size):
        out = None
        out_list = []
        ...
        for i in range(world_size):
            ...
            out_single = torch.matmul(x1.to(torch.float32), dequant.to(torch.float32))
            if out is None:
                out = out_single
            else:
    ...
    def test_npu_mm_all_reduce_base(self):
        ...
        for _ in range(world_size):
            ...
            x1_list.append(x1)
            x2_list.append(x2)
        expt_out_list = self._construct_excepted_result(x1_list, x2_list, scale, offset, world_size)
