...
class TestAlltoAllvGmmV2(TestCase):
    @classmethod
    ...
    @classmethod
    def _test_npu_alltoallv_gmm_v2(cls, rank, dtype, c2p, init_pg, input_list1, input_list2, expertTokenNum):
        gmmX, gmmWeight, mmX, mmWeight, is_trans_gmm_weight, is_trans_mm_weight, gmm_x_scale, gmm_weight_scale = input_list1
        ...
        goldenOut = TestAlltoAllvGmmV2._construct_golden_output(rank, dtype, input_list1, input_list2, expertTokenNum)
        
        gmmX = gmmX.to(torch.int8).npu()
        gmmWeight = gmmWeight.to(torch.int8).npu()
        
        if mmX is not None:
            mmX = mmX.to(torch.int8).npu()
        if mmWeight is not None:
            mmWeight = mmWeight.to(torch.int8).npu()

        gmmYOut, mmYOut, permuteOut = torch_npu.npu_alltoallv_gmm_v2(
            gmm_x=gmmX,
    ...
    def _construct_golden_output(cls, rank, dtype, input_list1, input_list2, expertTokenNum):
        ...
        num_tokens_per_local_expert = torch.tensor(np.sum(mc2_send_counts[rank].reshape(-1, e), axis=0).reshape(e)).npu().to(torch.int64)
        ...
        permuteGolden = TestAlltoAllvGmmV2.permute_with_npu(alltoAllvGolden, e, epWorldSize, expertTokenNum, rank)
    ...
    def test_npu_alltoallv_gmm_v2(self):
        dtype = torch.int8
        is_balance = True
        e = 32
        BS = 4096
        ...
        gmm_x_scale = torch.rand(e, 1).to(torch.int8)
        gmm_weight_scale = torch.rand(e, 1).to(torch.int8)
        ...
        self._test_multiprocess(
            TestAlltoAllvGmmV2._test_npu_alltoallv_gmm_v2,
            TestAlltoAllvGmmV2._init_dist_hccl,
            [gmmX, gmmWeight, mmX, mmWeight, is_trans_gmm_weight, is_trans_mm_weight, gmm_x_scale, gmm_weight_scale],
