...
class TestUpsamleTrilinear3DBackward(TestCase):
    ...
    def test_upsample_trilinear3d_backward_shape_format(self):
    ...
    def test_upsample_trilinear3d_backward_shape_format_scale(self):
        shape_format = self.get_format()
        for item in shape_format:
            cpu_input, npu_input = create_common_tensor(item[0], 0, 50)
            if cpu_input == torch.float16:
    ...
    def test_upsample_trilinear3d_backward_fp16(self):
