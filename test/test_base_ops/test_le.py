...
class TestLe(TestCase):
    ...
    def test_le_diff_device(self):
        input1 = cmp1 = torch.randn(5, 5)
        input2 = cmp2 = torch.tensor(1)
        diff_device_out = torch.le(input1.npu(), input2)
        diff_device_cmp = torch.le(cmp1, cmp2)
        self.assertRtolEqual(diff_device_out, diff_device_cmp)

        input1 = cmp1 = torch.tensor(1)
        input2 = cmp2 = torch.tensor(2)
        diff_device_out = torch.le(input1, input2.npu())
        diff_device_cmp = torch.le(cmp1, cmp2)
        self.assertRtolEqual(diff_device_out, diff_device_cmp)

    def test_le_first_arg_0d_cpu_tensor(self):
        cpu_a = torch.tensor(2.0)
        cpu_b = torch.tensor([1.0, 2.0, 3.0])
        npu_b = cpu_b.npu()
        cpu_output = torch.le(cpu_a, cpu_b)
        npu_output = torch.le(cpu_a, npu_b)
        self.assertEqual(cpu_output, npu_output.cpu())

    def test_le_inplace_second_arg_0d_cpu_tensor(self):
