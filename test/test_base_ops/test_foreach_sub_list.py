...
class TestForeachSubList(TestCase):
    ...
    def test_foreach_sub_list_out_float32_shpae_tensor_num(self):
    ...
    def test_foreach_sub_list_out_float16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float16")
            scalar = 5.0
            cpu_output = [(tensor1 - tensor2 * scalar) for tensor1, tensor2 in zip(cpu_tensors[0], cpu_tensors[1])]
            npu_output = torch._foreach_sub(npu_tensors[0], npu_tensors[1], alpha=scalar)

            self.assertRtolEqual(cpu_output, npu_output)

    @SupportedDevices(['Ascend910B'])
    def test_foreach_sub_list_out_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
    ...
    def test_foreach_sub_list_inplace_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalar = 5.0
    ...
    def test_foreach_sub_list_inplace_float16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float16")
            scalar = 5.0
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_sub_list_inplace_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "bfloat16")
            scalar = 5.0
