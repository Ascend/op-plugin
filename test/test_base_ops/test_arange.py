...
class TestArange(TestCase):
    def test_arange_default(self):
        ...
        for item in shape_format:
            cpu_output_default = torch.arange(
                item[0], item[1], item[2], device="cpu"
            ).numpy()
            npu_output_default = (
                torch.arange(item[0], item[1], item[2], device="npu").cpu().numpy()
            )
    ...
    def test_arange(self):
        ...
        for item in shape_format:
            cpu_output = torch.arange(
                item[0], item[1], item[2], dtype=item[3], device="cpu"
            ).numpy()
            npu_output = (
                torch.arange(item[0], item[1], item[2], dtype=item[3], device="npu")
                .cpu()
                .numpy()
            )
