...
class TestMaxPool1dWithIndices(TestCase):
    ...
    def test_max_pool1d_with_indices(self):
        shape_format_basic = [
            [np.float32, 0, [1, 2, 3], 3, 2, 0, 1, True],
            [np.float32, 0, [1, 2, 4], 2, 1, 0, 1, False],
            [np.float32, 0, [1, 2, 5], 3, 1, 0, 1, False],
            [np.float32, 0, [1, 2, 6], 3, 2, 0, 1, True],
        ]

        shape_format_padding = [
            [np.float32, 0, [1, 2, 7], 3, 1, 1, 1, False],
            [np.float32, 0, [1, 2, 7], 3, 2, 1, 1, False],
            [np.float32, 0, [2, 3, 11], 5, 2, 2, 1, False],
            [np.float32, 0, [2, 3, 11], 3, 1, 1, 1, True],
        ]

        shape_format_fp16 = [
            [np.float16, 0, [2, 4, 10], 2, 2, 0, 1, False],
            [np.float16, 0, [2, 4, 10], 3, 1, 1, 1, False],
            [np.float16, 0, [3, 6, 15], 3, 2, 0, 1, True],
            [np.float16, 0, [4, 8, 16], 2, 1, 0, 1, False],
            [np.float16, 0, [4, 8, 16], 4, 2, 1, 1, True],
        ]

        shape_format_large = [
            [np.float32, 0, [4, 5, 16], 3, 2, 1, 1, False],
            [np.float32, 0, [8, 4, 17], 3, 2, 0, 1, True],
            [np.float32, 0, [16, 8, 32], 2, 2, 0, 1, False],
            [np.float32, 0, [16, 16, 64], 4, 4, 0, 1, False],
            [np.float16, 0, [16, 16, 64], 3, 2, 1, 1, True],
        ]
        ...
        for item in shape_format:
            ...
            # Currently the indices returned by the max_pool1d function are of type int32.
