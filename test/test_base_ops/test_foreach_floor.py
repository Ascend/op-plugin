...
class TestForeachFloor(TestCase):
    ...
    def test_foreach_floor_out_float32_shpae_tensor_num(self):
        ...
        for tensor_num in tensor_num_list :
            ...
            cpu_output = torch._foreach_floor(cpu_tensors)
    ...
    def test_foreach_floor_out_float16_shpae_tensor_num(self):
        ...
        for tensor_num in tensor_num_list :
            ...
            cpu_output = [torch.from_numpy(np.floor(cpu_tensor)) for cpu_tensor in cpu_tensors]
            npu_output = torch._foreach_floor(npu_tensors)

            self.assertRtolEqual(cpu_output, npu_output)

    @SupportedDevices(['Ascend910B'])
    def test_foreach_floor_out_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [12, 62]
        for tensor_num in tensor_num_list :
    ...
    def test_foreach_floor_inplace_float32_shpae_tensor_num(self):
        ...
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_tensors(tensor_num, "float32")
            torch._foreach_floor_(cpu_tensors)
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_floor_inplace_bfloat16_shpae_tensor_num(self):
        tensor_num_list = [12, 62]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_tensors(tensor_num, "bfloat16")
            torch._foreach_floor_(cpu_tensors)
