...
class TestLess(TestCase):
    def cpu_op_exec(self, input1, input2):
        output = torch.less(input1, input2)
    ...
    def npu_op_exec_out(self, input1, input2, out1):
        torch.less(input1, input2, out=out1)
        output = out1.to("cpu")
        output = output.numpy().astype(np.int32)
        return output

    def cpu_op_exec_scalar(self, input1, scalar):
        output = torch.less(input1, scalar)
        output = output.numpy().astype(np.int32)
        return output
    ...
    def npu_op_exec_scalar_out(self, input1, scalar, out1):
        torch.less(input1, scalar, out=out1)
        output = out1.to("cpu")
        output = output.numpy().astype(np.int32)
        return output

    def less_tensor_out_result(self, shape_format):
    ...
    def test_less_tensor_out(self):
        ...
        self.less_tensor_out_result(shape_format)
    ...
    def test_less_scalar_out(self):
        ...
        self.less_scalar_out_result(shape_format)

    def less_result(self, shape_format):
        for item in shape_format:
    ...
    def less_scalar_result(self, shape_format):
        for item in shape_format:
            scalar = np.random.uniform(0, 100)
            cpu_input1, npu_input1 = create_common_tensor(item, 0, 100)
            if cpu_input1.dtype == torch.float16:
            ...
            cpu_output_inp_scalar = self.cpu_op_inplace_exec_scalar(cpu_input1, scalar)
            npu_output_inp_scalar = self.npu_op_inplace_exec_scalar(npu_input1, scalar)
    ...
    # scalar-----------------------------------------------------------------------
    def test_less_scalar_shape_format_fp16_1d(self):
        format_list = [-1, 0]
        shape_format = [[np.float16, i, 18] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp32_1d(self):
        format_list = [-1, 0]
        shape_format = [[np.float32, i, [18]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp16_2d(self):
        format_list = [-1, 0]
        shape_format = [[np.float16, i, [5, 8]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp32_2d(self):
        format_list = [-1, 0]
        shape_format = [[np.float32, i, [5, 8]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp16_3d(self):
        format_list = [-1, 0]
        shape_format = [[np.float16, i, [4, 16, 32]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp32_3d(self):
        format_list = [-1, 0]
        shape_format = [[np.float32, i, [4, 16, 32]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp16_4d(self):
        format_list = [-1, 0]
        shape_format = [[np.float16, i, [32, 3, 3, 3]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_scalar_shape_format_fp32_4d(self):
        format_list = [-1, 0]
        shape_format = [[np.float32, i, [32, 3, 3, 3]] for i in format_list]
        self.less_scalar_result(shape_format)

    def test_less_mix_dtype(self):
