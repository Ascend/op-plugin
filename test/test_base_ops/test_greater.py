...
class TestGreater(TestCase):
    ...
    def cpu_op_exec(self, input1, input2):
        output = torch.greater(input1, input2)
        output = output.numpy()
        return output

    def cpu_op_exec_out(self, input1, input2, input3):
        torch.greater(input1, input2, out=input3)
        output = input3.numpy()
        return output
    ...
    def npu_op_exec_out(self, input1, input2, output):
        torch.greater(input1, input2, out=output)
        output = output.to("cpu")
        output = output.numpy()
        return output

    def cpu_op_exec_scalar(self, input1, scalar):
        output = torch.greater(input1, scalar)
        output = output.numpy()
        return output

    def cpu_op_exec_scalar_out(self, input1, scalar, input2):
        torch.greater(input1, scalar, out=input2)
        output = input2.numpy()
        return output
    ...
    def npu_op_exec_scalar_out(self, input1, scalar, output):
        torch.greater(input1, scalar, out=output)
        output = output.to("cpu")
        output = output.numpy()
        return output

    def greater_tensor_out_result(self, shape_format):
    ...
    def test_greater_tensor_out(self):
        shape_format = [
        ...
        self.greater_tensor_out_result(shape_format)
    ...
    def test_greater_scalar_out(self):
        ...
        self.greater_scalar_out_result(shape_format)

    def test_greater_scalar_float32(self):
        format_list = [0]
        shape_list = [(5, 3), (2, 3, 4)]
        shape_format = [
    ...
    def test_greater_tensor_float32(self):
        format_list = [0]
        shape_list = [(5, 3), (2, 3, 4)]
    ...
    def test_greater_tensor_float16(self):
        format_list = [0]
        shape_list = [(5, 3), (2, 3, 4)]
    ...
    def test_greater_inplace_float32(self):
        format_list = [0, 3]
        shape_list = [(5, 3), (2, 3, 4)]
        shape_format = [[[np.float32, i, j], [np.float32, i, j]]
                        for i in format_list for j in shape_list]
    ...
    def test_greater_inplace_float16(self):
        format_list = [0, 3]
        shape_list = [(5, 3), (2, 3, 4)]
        shape_format = [[[np.float16, i, j], [np.float16, i, j]]
                        for i in format_list for j in shape_list]
    ...
    def test_greater_inplace_scalar_float32(self):
        format_list = [0]
        shape_list = [(5, 3), (2, 3, 4)]
        shape_format = [
            [np.float32, i, j] for i in format_list for j in shape_list
    ...
    def test_greater_inplace_scalar_float16(self):
        format_list = [0]
        shape_list = [(5, 3), (2, 3, 4)]
        shape_format = [
            [np.float16, i, j] for i in format_list for j in shape_list
    ...
    def test_greater_mix_dtype(self):
    ...
    def test_greater_first_arg_0d_cpu_tensor(self):
        cpu_a = torch.tensor(2.0)
        cpu_b = torch.tensor([1.0, 2.0, 3.0])
        npu_b = cpu_b.npu()
        cpu_output = torch.greater(cpu_a, cpu_b)
        npu_output = torch.greater(cpu_a, npu_b)
        self.assertEqual(cpu_output, npu_output.cpu())

    def test_greater_inplace_second_arg_0d_cpu_tensor(self):
        cpu_a = torch.tensor([1.0, 2.0, 3.0])
        cpu_b = torch.tensor(2.0)
        npu_a = cpu_a.clone().npu()
        cpu_a.greater_(cpu_b)
        npu_a.greater_(cpu_b)
