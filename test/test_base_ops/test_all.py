...
class TestAll(TestCase):
    ...
    def test_all_shape_format(self):
        ...
        for item in shape_list:
            ...
            self.assertRtolEqual(
                cpu_output.astype(np.int32), npu_output.astype(np.int32)
            )
    ...
    def test_alld_shape_format(self):
        ...
        for item in itertools.product(dtype_list, format_list, shape_list):
            ...
            self.assertRtolEqual(
                cpu_output.astype(np.int32), npu_output0.astype(np.int32)
            )
            self.assertRtolEqual(
                cpu_output.astype(np.int32), npu_output1.astype(np.int32)
            )
