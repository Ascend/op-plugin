...
class TestBitwiseRightShift(TestCase):
    ...
    def test_bitwise_right_shift_tensor_scalar(self, device="npu"):
        format_list = [0]
        shape_list = [(256, 32, 56)]
        shape_format = [[np.int32, i, j] for i in format_list for j in shape_list]
        for item in shape_format:
    ...
    def test_bitwise_right_shift_scalar_tensor(self, device="npu"):
        input_value = 512
    ...
    def test_bitwise_right_shift_tensor_scalar_out(self, device="npu"):
        shape_format = [
            [[np.int32, 0, [256, 128, 7, 7]], [np.int32, 0, [256, 128, 7, 7]]],
            [[np.int32, 0, [2, 3, 3, 3]], [np.int32, 0, [2, 3, 3, 3]]],
            [[np.int32, 0, [128, 232, 7, 7]], [np.int32, 0, [128, 232, 7, 7]]],
    ...
    def test_bitwise_right_shift__tensor_scalar(self, device="npu"):
        format_list = [0]
        shape_list = [(256, 32, 56)]
        shape_format = [[np.int32, i, j] for i in format_list for j in shape_list]
        for item in shape_format:
            cpu_input1, npu_input1 = create_common_tensor(item, 0, 100)
            shift_value = 2
            cpu_input1.bitwise_right_shift_(shift_value)
            npu_input1.bitwise_right_shift_(shift_value)
