...
class TestForeachAddcmulScalarList(TestCase):
    ...
    def create_input_scalars(self, tensor_nums, dtype):
        ...
        for i in range(tensor_nums):
            m = float(random.randint(-100, 100))
            if m == 0:
                m = 2.4
            sacalars.append(m)
    ...
    def test_foreach_addcmul_scalar_list_out_float32_shpae_tensor_num(self):
    ...
    @SupportedDevices(['Ascend910B'])
    def test_foreach_addcmul_scalar_list_inplace_float32_shpae_tensor_num(self):
        tensor_num_list = [20, 50]
        for tensor_num in tensor_num_list :
            cpu_tensors, npu_tensors = self.create_input_tensors(tensor_num, "float32")
            scalars = self.create_input_scalars(tensor_num, "float32")
