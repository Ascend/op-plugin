...
本目录提供 Dockerfile 及构建脚本，用于生成 OpPlugin 的构建与开发镜像：`builder` 镜像提供包含 OpPlugin 源码的 TorchNPU wheel 编译环境（不含 CANN），`dev` 镜像在此基础上叠加 CANN 运行环境。
...
- **builder**（默认）：用于编译包含当前 OpPlugin 源码的 TorchNPU wheel，不含 CANN
