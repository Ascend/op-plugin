...
本目录提供 Dockerfile 及构建脚本，用于生成 OpPlugin 的构建与开发镜像：`builder` 镜像提供包含 OpPlugin 源码的 torch_npu wheel 编译环境（不含 CANN），`dev` 镜像在此基础上叠加 CANN 运行环境。
...
- **builder**（默认）：用于编译包含当前 OpPlugin 源码的 torch_npu wheel，不含 CANN
- **dev**：基于 builder，叠加 CANN 运行环境；继承全部编译工具链，可在容器内直接编译和调试 OpPlugin
...
| 参数 | 说明 |
|------|------|
| `-p, --python VERSION` | Python 版本：3.10、3.11、3.12、3.13（默认：3.10） |
| `--torch-version VER` | PyTorch 版本，格式 `x.x.x`（如 `2.13.0`）或 dev 版本（如 `2.13.0.dev20260610`），默认：`2.13.0` |
...
仅需挂载源码，无需 NPU 驱动透传。以下命令须在 `op-plugin/docker/devel` 目录下执行，与手动构建步骤的工作目录一致：

```Shell
docker rm -f op-plugin-builder 2>/dev/null

docker run -d --rm \
    --name op-plugin-builder \
    -v $(pwd)/../..:/home/op-plugin \
    -e PY_VERSION=3.10 \
    op-plugin-builder:v1 \
    tail -f /dev/null
```
...
```Shell
docker exec op-plugin-builder npu-smi info
```
...
```Shell
docker exec -it op-plugin-builder bash
```

进入容器后，切换到 OpPlugin 源码目录：
...
- **工作目录**：需进入 `op-plugin\docker\devel` 目录。
- **路径分隔符**：使用反斜杠 `\`，而非正斜杠 `/`。
