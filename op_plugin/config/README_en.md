...
<!-- md-trans-meta sourceCommit=c6d785c77c92c77d7fc800bb1620ff277b5750a9 translatedAt=2026-08-11T01:31:18.425Z pushedAt=2026-08-11T12:02:23.800Z -->
...
│   │   ├── deprecated.yaml # Configuration file for generating deprecation alarms
...
namespace acl_op {
// Function parameters differ between 1.11 and 2.0 or later, so use a macro to control
...
#endif
// Code for 2.0 and later versions is consistent
...
- Configure forward and backward operator bindings.
...
- `exec`: Parameters passed to `EXEC_NPU_CMD`. If the order of all parameters other than `aclnnname` is the same as that in the ATen IR (with the `out` parameter of the original function excluded), only `aclnnname` needs to be configured. For example, the `exec` field of `abs` can be configured in either of the following ways.
...
For example, for `abs`, the original API and the `out` API have the same `out` configuration and `exec` definition, so they can inherit these settings through `structured_inherit`.
...
## Adding Deprecation Alarms for Custom NPU APIs

### YAML Configuration

When a custom ATen API needs to be deprecated, deprecation alarms can be generated automatically by configuring `op_plugin/config/deprecated.yaml`.
...
If `replace` is specified, the generated alarm follows this format:

```python
f'TORCH_WARN_ONCE("{name} is deprecated and will be removed in future version. Use {replace} instead.");'
```

`name` does not include the overload name. For example, for `npu_broadcast.out`, the generated alarm is:

```python
'TORCH_WARN_ONCE("npu_broadcast is deprecated and will be removed in future version. Use torch.broadcast_to instead.");'
```

If `replace` is not specified, the generated alarm follows this format:

```python
f'TORCH_WARN_ONCE("{name} is deprecated and will be removed in future version.");'
```

Likewise, the overload name is omitted from `name`. For example, for `npu_nms_rotated`, the generated alarm is:
