...
at::Tensor npu_quant_conv2d_out(
  ...
  TORCH_CHECK(
      output_dtype.value() == static_cast<int64_t>(at::ScalarType::Half), "only support float16 as outputdtype");
...
at::Tensor npu_quant_conv2d(
  ...
  TORCH_CHECK(
      dilations.size() >= ATTRS_DIM, "dilation has to contain more than 2 elements, but got ", dilations.size());
