...
at::Tensor npu_matmul_compress_dequant(
    const at::Tensor& x1,
    const at::Tensor& x2,
    const at::Tensor& compress_index,
    const at::Tensor& bias,
    const at::Tensor& scale,
    const c10::optional<at::Tensor>& offsetW,
    c10::optional<int64_t> offsetX) {
  static const bool is_aclnn_available = check_aclnn_kernel_available("aclnnMatmulCompressDequant");
  TORCH_CHECK(
      is_aclnn_available,
  ...
  TORCH_CHECK(
      offset_x_val == 0, "offsetX currently only supports 0, but got ", offset_x_val, OPS_ERROR(ErrCode::PARAM));

  TORCH_CHECK(
      x1.dim() == MINIMUM_SHAPE_SIZE, "x1 must have 2 dimensions, but got ", x1.dim(), OPS_ERROR(ErrCode::PARAM));
  TORCH_CHECK(
      scale.dim() == MINIMUM_SHAPE_SIZE,
      "scale must have 2 dimensions, but got ",
      scale.dim(),
      OPS_ERROR(ErrCode::PARAM));
  TORCH_CHECK(
      bias.dim() == MINIMUM_SHAPE_SIZE, "bias must have 2 dimensions, but got ", bias.dim(), OPS_ERROR(ErrCode::PARAM));
  ...
  at::Tensor result =
      npu_preparation::apply_tensor_with_format(output_size, x1.options().dtype(at::kHalf), ACL_FORMAT_ND);
