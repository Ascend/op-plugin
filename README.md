...
安装时需要安装系统依赖及官方PyTorch框架，建议使用torch_npu提供的docker镜像进行编译，依赖安装和镜像使用指导可参考[Ascend Extension for Pytorch](https://www.hiascend.com/document/detail/zh/Pytorch/730/configandinstg/instg/docs/zh/installation_guide/compilation_installation_using_source_code.md)。
...
2. 编译生成插件的二进制安装包。
   ...
   cd op-plugin
   ```
   执行编译构建，当前支持torch_npu 2.6/2.7/2.8/2.9/2.10/2.11/master版本。
   ```
   bash ci/build.sh --python=3.11 --pytorch=v2.7.1
   ```
   >**须知：**<br>
   >请注意，编译时gcc版本遵循如下约束：
   ...
   | 参数      | 取值范围                                                           | 说明                  | 缺省值    | 备注                                |
   |---------|----------------------------------------------------------------|---------------------|--------|-----------------------------------|
   | PyTorch | v2.6.0, v2.7.1, v2.8.0, v2.9.0, v2.10.0, v2.11.0, master | 指定编译过程中使用的PyTorch版本 | master | master对应的PyTorch版本为2.12            |
   | Python  | 3.9, 3.10, 3.11, 3.12                                           | 指定编译过程中使用的Python版本  | 3.10    | 编译基于PyTorch2.9及后续版本时，不支持Python3.9 |
   ...
   | v2.9.0       | 3.10, 3.11, 3.12     | 13.3               |
   | v2.10.0      | 3.10, 3.11, 3.12     | 13.3               |
   | v2.11.0      | 3.10, 3.11, 3.12     | 13.3               |
   | master       | 3.10, 3.11, 3.12     | 13.3               |
...
3. 完成编译后，安装dist目录下生成的插件torch\_npu包，如果使用非root用户安装，需要在命令后加`--user`。
   ...
   # 典型的whl包名类似：torch_npu-2.7.1.post3-cp39-cp39-manylinux_2_28_aarch64.whl
   ```
...
#### 快速验证
 ...
 可以通过以下样例快速验证是否成功安装，具体支持的API列表可参考[自定义API参考](https://gitcode.com/Ascend/op-plugin/blob/master/docs/zh/custom_APIs/menu_Pytorch_API.md)。
