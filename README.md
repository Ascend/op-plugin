...
OpPlugin算子插件的编译、使用依赖Ascend Extension for PyTorch。因此，在编译OpPlugin之前，需要了解、安装Ascend Extension for PyTorch。使用手册可参考昇腾社区[Ascend Extension for Pytorch](https://gitee.com/ascend/pytorch/blob/master/README.zh.md)。
...
2. 编译生成插件的二进制安装包。
   ...
   ```
   git clone https://gitee.com/ascend/op-plugin.git -b 6.0.0
   cd op-plugin
   ```
   执行编译构建，当前支持torch_npu 2.1/2.3/2.4/版本
   ```
   bash ci/build.sh --python=3.8 --pytorch=v2.1.0-6.0.0
   ```
   请注意，编译时gcc版本遵循如下约束：
   ...
   | 参数      | 取值范围                                   | 说明                  | 缺省值    | 备注                                |
   |---------|----------------------------------------|---------------------|--------|-----------------------------------|
   | pytorch | v2.1.0-6.0.0, v2.3.1-6.0.0, v2.4.0-6.0.0| 指定编译过程中使用的pytorch版本 | master | 需要显示指定pytorch版本，避免使用缺省值master            |
   | python  | 3.8, 3.9, 3.10, 3.11                   | 指定编译过程中使用的python版本  | 3.8    |  |
