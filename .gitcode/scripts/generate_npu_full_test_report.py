...
def discover_shard_files(
    ...
    def parse_shard_filename(path: Path, suffix_pattern: str) -> Optional[Tuple[str, int]]:
        ...
        if match:
            ...
            return None
        match = re.match(r"shard_(reg|custom)-(\d+)_" + suffix_pattern, stem)
        if match:
...
def build_file_to_shards_map(cases_shards_dir: Path) -> Dict[str, List[str]]:
    ...
    # Pattern: {test_type}_cases_shard_{num}.json or {test_type}_cases.json (single shard)
    shard_files = list(cases_shards_dir.glob("*_cases_shard_*.json"))
    shard_files += [p for p in cases_shards_dir.glob("*_cases.json")
                    if not p.name.endswith("collection_summary.json")]
...
def main():
    ...
    if cases_results:
        ...
        if args.cases_by_file_dir:
            ...
            # 常规用例的 by_file 文件名随收集阶段的 test_type 变化:
            # 不区分 cpu 时为 regular_npu_cases_by_file.jsonl, 兼容 regular_ 前缀
            reg_jsonl_path = cases_by_file_dir / "regular_cases_by_file.jsonl"
            if not reg_jsonl_path.exists():
                candidates = sorted(cases_by_file_dir.glob("regular*_cases_by_file.jsonl"))
                if candidates:
                    reg_jsonl_path = candidates[0]
    ...
    if cases_results and args.cases_by_file_dir:
        ...
        # Process regular cases (文件名兼容: regular_cases_by_file.jsonl 或 regular_npu_cases_by_file.jsonl)
        reg_jsonl_path = cases_by_file_dir / "regular_cases_by_file.jsonl"
        if not reg_jsonl_path.exists():
            candidates = sorted(cases_by_file_dir.glob("regular*_cases_by_file.jsonl"))
            if candidates:
