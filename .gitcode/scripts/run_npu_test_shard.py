...
Each worker subprocess runs pytest.main() for multiple same-file cases:
    - Cases are sorted by test file and grouped into batches (max 50 per batch)
...
def run_tests_with_tasks_concurrent(
    ...
    log_queue.put({
        ...
        "content": (
            ...
            "Execution mode: pytest.main() per case, batched by file (max 50/batch)\n"
            "=" * 80 + "\n\n"
        ),
    })
    ...
    # Sort and batch tasks: group same-file cases, max 50 per batch
...
def _execute_worker_batch(
    ...
    while remaining_cases:
        ...
        try:
            ...
            # 空闲兜底: 进程超过单用例超时 + 30s 仍无任何输出即判定卡死,
            # 杀掉后继续下一个用例 (单用例超时 180s, 兜底为 210s)
...
def _run_mode_cases_json(args, test_dir: Path, report_dir: Path, script_dir: Path,
    ...
    if shard_type.startswith("distributed"):
        if args.cards_per_test > 0:
            ...
            print(f"Execution mode: SERIAL (1 batch at a time, all cards visible)")
    else:
