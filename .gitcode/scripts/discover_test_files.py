...
# ==============================================================================
# Content-based Distributed Classification (Step 1.5)
# ==============================================================================


# 分布式风格标记: 文件不在 test/distributed/ 目录下, 但用例内部依赖多卡通信
# (HCCL 进程组初始化 / 多卡装饰器 / run_tests 多进程拉起)。这类用例内部会
# 自拉多进程并占用多张卡, 必须串行独占执行, 若按常规用例并行调度,
# ASCEND_RT_VISIBLE_DEVICES 会被限定为单卡, 多卡用例将被跳过。
_DISTRIBUTED_CONTENT_PATTERN = re.compile(
    r"dist\.init_process_group|@skipIfUnsupportMultiNPU|run_tests\(\s*world_size\s*="
)

_content_classify_cache: Dict[str, bool] = {}


def is_distributed_style_file(file_path: Path) -> bool:
    """Check whether a test file's content contains distributed-style markers (cached)."""
    key = str(file_path)
    if key not in _content_classify_cache:
        try:
            source = file_path.read_text(encoding="utf-8", errors="replace")
        except OSError:
            source = ""
        _content_classify_cache[key] = bool(_DISTRIBUTED_CONTENT_PATTERN.search(source))
    return _content_classify_cache[key]


def classify_by_content(test_files: List[str], test_dir: Path) -> Tuple[List[str], List[str]]:
    """
    Classify test files into distributed-style / regular by file content.

    Args:
        test_files: List of file paths (with 'test/' prefix)
        test_dir: Test root directory (the directory that 'test/' refers to)

    Returns:
        (distributed_files, regular_files), order consistent with input.
    """
    distributed: List[str] = []
    regular: List[str] = []
    for f in test_files:
        rel = f[5:] if f.startswith("test/") else f
        if is_distributed_style_file(test_dir / rel):
            distributed.append(f)
        else:
            regular.append(f)
    return distributed, regular
...
def discover_classified(
    test_dir: Path,
    case_paths_config: Optional[str],
) -> Tuple[List[str], List[str], Dict]:
    """
    Discover all test files and classify them into distributed / regular.

    Distributed classification rules:
      1. Path-based: files under test/distributed/ directory
      2. Content-based: files containing distributed-style markers
         (dist.init_process_group / skipIfUnsupportMultiNPU /
         run_tests(world_size=...)), i.e. cases that internally spawn
         multi-process workers and require multiple NPU cards.
    All other files are treated as regular cases.

    Path rules (whitelist/blacklist) are applied before classification.

    Returns:
        Tuple of (distributed_files, regular_files, metadata_dict)
    """
    # Step 1: Discover all test files
    all_test_files = discover_raw_test_files(test_dir)
    total_count = len(all_test_files)

    # Step 3: Apply whitelist/blacklist rules (before classification)
    config_path, whitelist, blacklist = load_case_path_rules(case_paths_config)
    rules_selected, rules_excluded = apply_case_path_rules(all_test_files, whitelist, blacklist)

    # Step 4: Classify into distributed / regular
    dir_distributed = [f for f in rules_selected if f.startswith("test/distributed/")]
    dir_regular = [f for f in rules_selected if not f.startswith("test/distributed/")]
    style_distributed, style_regular = classify_by_content(dir_regular, test_dir)

    distributed = sorted(dir_distributed + style_distributed)
    ...
    metadata = {
        ...
        "rules_excluded": len(rules_excluded),
        "case_paths_config": config_path,
        "dir_distributed": len(dir_distributed),
        "style_distributed": len(style_distributed),
        "distributed_total": len(distributed),
        "regular_total": len(regular),
        "style_distributed_files": style_distributed,
    }

    return distributed, regular, metadata
...
def parse_args():
    ...
    parser.add_argument(
        "--test-type",
        type=str,
        choices=["distributed", "regular", "auto"],
        default="auto",
        help="Test type: 'distributed' for distributed tests only, 'regular' for other "
             "tests only, 'auto' (default) to discover all files and classify them into "
             "distributed / regular automatically",
    )
    parser.add_argument(
        "--regular-output",
        type=str,
        help="With --test-type auto: output file path for regular test file list",
    )
    parser.add_argument(
        "--case-paths-config",
        type=str,
...
def _write_list(files: List[str], path: Path) -> None:
...
def main():
    ...
    if args.test_type == "auto":
        # Auto-classified mode: discover all files and split into
        # distributed / regular lists
        distributed_files, regular_files, metadata = discover_classified(
            test_dir=test_dir,
            case_paths_config=args.case_paths_config,
        )

        if args.output:
            dist_path = Path(args.output).resolve()
            _write_list(distributed_files, dist_path)
            if args.verbose:
                print(f"Written {len(distributed_files)} distributed test files to: {dist_path}")
        else:
            sys.stdout.write("\n".join(distributed_files) + ("\n" if distributed_files else ""))

        if args.regular_output:
            reg_path = Path(args.regular_output).resolve()
            _write_list(regular_files, reg_path)
            if args.verbose:
                print(f"Written {len(regular_files)} regular test files to: {reg_path}")
        ...
        if args.verbose:
            print(f"\nDiscovery Summary (auto classification):")
            print(f"  Test directory: {test_dir}")
            print(f"  Total files scanned: {metadata['total_files']}")
            print(f"  After rules filter: {metadata['rules_selected']} selected, "
                  f"{metadata['rules_excluded']} excluded")
            print(f"  Distributed (directory): {metadata['dir_distributed']}")
            print(f"  Distributed (style-detected): {metadata['style_distributed']}")
            print(f"  Distributed total: {metadata['distributed_total']}")
            print(f"  Regular total: {metadata['regular_total']}")
        return
